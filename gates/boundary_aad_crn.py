...
everything:
    ...
    AAD only            the frozen-decision gradient derivus reports today
...
def cva(spot, batch=PATHS, seed=SEED):
    ...
    _, out = derivus.run_cmc(cfg, prec=fixture.DTYPE,
                              overrides=fixture._params(False, seed=seed, batch=batch))
    return float(out['Results']['cva'])


def aad_delta(with_correction, batch=PATHS, seed=SEED, bandwidth=BANDWIDTH):
    """No setting yields greeks WITHOUT the correction any more, so the uncorrected gradient is
    obtained the way test_correction_moves_gradients_but_not_the_forward gets it - by suppressing
    the term at the SET."""
    params = fixture._params(True, seed=seed, batch=batch)
    ...
    try:
        _, out = derivus.run_cmc(fixture._cfg(MTA), prec=fixture.DTYPE, overrides=params)
    finally:
