...
the deals loaded; `out['Results'].keys()` enumerates everything that's available.
...
| `POST` | `/book/deals` | book, amend or delete one deal — validated BEFORE an atomic write; a refusal is `{"written": false, "refused": […]}` and touches nothing. A `reference` beside the `deal_path` of an amendment or a delete names the deal the client read there, and a path another host's write has moved refuses by name. Under a spine home a booking carries `quantity` - the position change in units of the deal, 1 booking it as written - and `execution_reference`, and may name the `agreement` whose netting set it sits under (the counterparty then the agreement's entity), the `portfolio` it sits in - a path whose top node is the book, `<book>/Rates/EM`, the book itself by default, and any other top node refused by name - and the `price` it was done at |
...
| `GET` | `/calculations` | the desk's own named calculations — `{calculations: {name: block}}`, each a `Calculation` block (`Object` the type, every other key one of its declared fields), kept in `DV_HOME` and never in the book or the record |
| `POST` | `/calculations` | `{name, calculation}` saves one, a null `calculation` removing it; judged against the type's declarations, one level into a container included, a judged block answering `{written: false, refused: [...]}` with the file untouched |
| `POST` | `/calculations/run` | `{name, deal_path?, actor?}` — the live book, or the subtree at `deal_path`, under the named calculation merged over the book's own exactly as `/book/price` merges overrides, so one set of dials is one run; curiosity lane, nothing recorded; `{result_id, status}` |
...
| `GET` | `/book/positions` | the record's positions standing AT THE HEAD, one per instrument, agreement and portfolio - the counterparty, the NET quantity in units of the deal and the clips behind it - each beside where the book file holds it: `deal_paths` the nodes carrying that instrument under that agreement's set, `reference` and `object` the first one's. A position closed to zero or amended onto other terms stands no more, and one the file does not hold answers no path; the two grouped views of the web UI, by portfolio and by client, read it with the paper. 404 where no spine home is configured |
...
| `POST` | `/book/entities` | declare a legal entity the book trades with — `{entity, name, parent?, actor}`, answering `{recorded: {lsn}, entity}`: its id as the deployment's legal system knows it, the name a reader is shown, and the entity it is grouped under. The act is `document`, enforced by the record in its own words; a second declaration under one id restates it; 404 where no spine home is configured |
| `GET` | `/book/entities` | `{entities}` — every legal entity the record declares, with the parent each is grouped under |
| `POST` | `/book/agreements` | declare an agreement with a declared entity — `{agreement, entity, kind, terms, actor}`, answering `{recorded: {lsn}, agreement, terms}`, `terms` the address they are stored at. The terms are the `NettingCollateralSet` every position under the agreement is compiled into, judged by the engine before anything appends: a block that is not a netting set, one carrying positions, one stating a balance or a holding (settlement state, never paper) and one its own declarations refuse are each a 422 by name, and so is an entity nobody declared |
| `GET` | `/book/agreements` | `{agreements}` — every agreement the record declares with its entity, kind and terms, and `terms_hash` the address they were read from |
