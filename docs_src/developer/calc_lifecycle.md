...
A step *inside* phase 2: `_build_factor_state` → `_init_shared_mem` → `get_cholesky_decomp`, the only place `process_ofs` is populated, ahead of the precalculate loop that reads it. It iterates `stoch_factors.items()` in topological order; per factor `value.correlation_name` returns `(corr_type, [sub_factor_tuples])`, `process_ofs.setdefault(key, len(correlation_factors))` records the row offset of this factor's random substream, and each sub-factor appends a `Factor(corr_type, key.name+sub)`. The symmetric matrix is built from `Correlations`, healed to PSD if needed, and `torch.linalg.cholesky`'d. Between the two: **each factor's `correlation_dilution(intervals)`**, the share of a declared TOTAL-RETURN correlation its own return realises on the calculation's own scenario intervals. One exactly for every process but a mixed one, and where every dilution is one the matrix is not touched at all — the same object, bit for bit. Where any is under one, `utils.scale_correlation` refuses every pair whose `|rho|` exceeds `D_i D_j` (naming the pair, the declared number and the maximum), divides the rest by `D_i D_j`, resets the diagonal and logs the largest move at INFO; the existing healing loop then repairs the scaled matrix, and a repair moving any entry by more than `Correlation_Repair_Tolerance` (default 0.02) refuses by name — a scaled matrix that is no longer positive semi-definite is no longer the desk's matrix.
