...
| | Where | Status |
| --- | --- | --- |
| Collateralised autocall boundary correction | boundary registration + `gross_to_net` | An autocall settles a coupon when it fires, and a collateralised exposure reads that cash through `C_ts_te` - but the boundary counterfactual flips only the MTM (`gross_to_net` takes no cashflow delta), so the collateralised CVA spot delta reads LOW: 10.50% on the JSON gate (flatness 4.40%), 18.84% on the Config twin (flatness 1.37%), and removing the cash channel from BOTH estimators takes the same fixtures to agreement - the residual is the cash leg and nothing else. Strict xfail: `test_autocall_json.py::test_a_collateralised_cva_delta_carries_the_settled_coupon`. The fix is a per-decision cashflow delta on the registration and a `gross_to_net` that accepts it. |
