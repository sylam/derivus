...
| Extendable forward: boundary flux, mirror booking | `pv_MC_ExtendableForward` | The deal ships forward-value complete (Black-oracle gates, both styles). The boundary registration is **DONE**: reconstructed decisions register a `LatchedBoundarySet` whose alive branch is the facts-only world and whose dead branch is the survived-weighted `pending` head, both derived from the pricer's own `value = fixed + state * live` split - the `EXTENDABLE_LATCH` organ reads reconstruction at 5.5e-8 relative, and the CVA delta closes from -3.17% (registration suppressed) to -0.07% against its CRN ladder (`test_the_cva_delta_carries_the_extension_flux`). REMAINING declared limits: (1) the settled-cash channel under a CSA is not registered, BY MEASURED RULING: across four amplifying documents (ITM extension, vol 0.30 / 20-day margin period, a one-fixing zero-lag tail that makes the flipped object nearly pure cash) the residual reads +0.25% / -0.02% / +0.26% / +0.03% against CRN ladders that resolve no finer - the flipped payments' exposure rides the VALUE side (scored exactly by `pending`) until settlement, then one hazard-weighted margin window; the pay-on-surviving `cash_alive` design is recorded in `test_a_collateralised_cva_delta_carries_the_surviving_cash` and waits for a document that can falsify it. (2) The mirror booking is **DONE (2026-09-02)**. `Exercised_By` (declared, default `Bank` = the reporter; every existing fixture byte-identical, hex-checked independently on a 9216-entry CMC profile) splits the payoff's `forward_sign` from the exercise rule's `decide_sign = forward_sign × exercise_sign`, which orients the OSS truncation, the reconstructed decision and its latch gap, while the backward pass's `relu` becomes the exerciser's own fold `es·relu(es·c)` — max on the reported book, min on the mirror. The boundary never moves: it is where the REPORTED continuation crosses zero, which no exerciser moves, so a deal and its mirror are one exact negation — all four style×side pairs sum to EXACTLY 0.0 at valuation and row-by-row across the CMC profile; the max-for-min mutant reads 4.1%/34%, the flipped truncation 174%, the reconstruction 79% (invisible at base valuation — the profile gate is what catches it). The written side prices its own Black put at 1.0e-3 with parity closing to 4.5e-4, and the mirror's CVA delta carries MORE flux than the book's (−18.4% suppressed against 3.2%), its exposure sitting on the terminated paths. The build also found and fixed a HARNESS defect: `_job` shared `FACTORS` by reference, so every CVA ladder rung permanently re-bound `FxRate.EUR` (and minted an unauthored `SurvivalProb.CPTY`) for every later job in the process — file-order readings from that file predating the fix are suspect, and the collateralised gate now reads its documented +0.2456%. (3) The rolling backward pass carries a small one-signed smoothing bias (Gauss-Hermite over the relu kink, interpolated); single-decision measures inside 5e-3 of Black and the multi-decision case is bounded by the dominance gates only — the `Boundary_*` valuation options are the accuracy dials. |
...
| ~~`pv_partial_barrier_option`~~ **REVIEWED AND CLOSED** | `pricing` | Three structural defects and two formula defects, all fixed, gated by an independent bridge-corrected Monte Carlo oracle (`test_partial_barrier_json.py`, worst 0.43% over all eight direction/window configurations). Structural: (1) the suspected NaN was real - rows at/past `Barrier_Limit_Date` fed a non-positive window into `sqrt` (inf-inf in the e-terms at zero); the clamp is also the valuation there, both window types collapsing to the right limit of the same formula. (2) The outer `touched` monitoring ignored the window entirely - a crossing outside it knocked anyway, measured -87% on realised payoffs. (3) Down + end-window + continuous monitoring NEVER PRICED: a float barrier made `strike < barrier` a python bool and `torch.where` refused - silent skip, NaN mark. Formula (adjudicated by workflow against exact-BivN + 4M-path MC): the eta==0 B1 branch had an INVERTED strike-vs-barrier selection and wrong e3/e4 signs in the reflected terms (max error 8.69 on 100-spot cases, negative out-option prices) - unreachable from the FX deal's declared Barrier_Type values, fixed anyway; the suspected eta==-1 argument slip was REFUTED (bivariate-normal symmetry). ApproxBivN edge ledger, all currently unreachable from this sole consumer post-clamp (rho strictly inside (0,1)): NaN grads for rho=0 rows in mixed batches, silent zero rho-grad at exact rho=0, rho=+-1 NaN-or-wrong, P/Q=+-inf rho-grad NaN; true worst accuracy ~7e-4 not the claimed 4 decimals. The two STILL-OPEN items were taken 2026-09-02 and read differently now. **(a) REFUTED where the bridge lives, RE-AIMED, and OPT-IN where it is real.** The `touched` "latch" is not one wherever an interval variance exists: `barrier_touched`'s bridge branch is CONTINUOUS in the spot (an endpoint beyond the barrier drives the bridge term to exactly one, so `maximum(beyond, crossed) == crossed` identically) and the CVA delta already agrees with its ladder — −2.742147 against a best rung of −2.730047, 0.44% at 1.02% flatness, with NOTHING registered; registering there would double count. The flux is real only where `get_fx_barrier_underlying` publishes no rate (the quote leg itself simulated — an FX barrier reported off a third currency), where the test collapses to a 0/1 endpoint indicator. A `LatchedBoundarySet` registers exactly there — behind the NEW declared field `Boundary_AAD_Window_Touch` (default `No`, bit-identical off), because the registration decides the SIGN (−2.2467692 with, +0.5207422 without) while its MAGNITUDE is unestablished: the CRN oracle scatters 88% of its own median and is not even sign-unanimous across path counts, and the fixture cannot be enriched (the row below on `add_grid_dates`). **(b) CLOSED, and it was worth EXACTLY nothing in both directions**: the rebate moved the mark by 0.0000 in all eight configurations against an oracle putting it at 14–34, the KO's pre-hit leg missing by the same omission as the KI's pad. `pricing.partial_window_rebate` values both legs over the window — and its own review caught the START-window branch returning `never` = −1.05 and `at_hit` = 2.14 on the beyond side (the closed forms assume the LIVE side; reachable at base valuation on a deal whose spot is past the level, and on a SEASONED window, `limit_years < 0`): both shapes now resolve exactly — touched pays its rebate once at par, a closed window pays 0 (KO) or the discounted certainty (KI, 48.039472 = 50·e^{−0.04} to 1e-9) — swept over 6912 parameter readings with worst start-window excursion outside [0,1] exactly 0.0. Gates: the eight-configuration oracle table (worst 0.56%), the aging gate (5.62 against 39.9 with the term absent), and the touched-or-closed decathlon whose guard-removal mutant dies by 61–214% of the rebate. NOTE the sibling correction: `pv_barrier_option`'s (full-window) KI rebate DOES carry pre-expiry value through its `E` term — measured 41.65 vs 21.28 — so the missing-rebate defect was this pricer's alone. |
| The start-window OPTION leg prices the live side only, so a seasoned knock-out marks NEGATIVE | `pricing.getpartialbarrierpayoff` (start-window arm) | Found by the rebate fix's own review (2026-09-02), measured PRE-EXISTING at HEAD — not this wave's: a seasoned `Up_And_Out` whose barrier sits below spot marks **−6.141370** where the vanilla is 85.23 — a negative price for a bought knock-out call — and its KI marks 91.44. Same root as the rebate half just fixed (the closed forms assume the spot is on the live side of a live window; nothing guards the beyond side or a closed window), fixed there and NOT here because the option leg needs its own resolution table and its own oracle. Recorded in `test_a_touched_or_a_closed_window_pays_its_rebate_exactly_once_or_not_at_all`'s docstring so nobody reads −6.14 as blessed. |
| `FXPartialTimeBarrierOption` monitors its window on three dates | `instruments.FXPartialTimeBarrierOption` | It declares `path_dependent = True` but never overrides `add_grid_dates`, so it contributes only `{Expiry_Date, Barrier_Limit_Date}` — as the only deal in a portfolio its dates ARE the reporting grid, whatever `Time_grid` asks for, and rows between them are linear INTERPOLATION of the two closed-form evaluations, never the closed form (measured: a day-250 row is `14.7787 × (1 − 68/183)` to the bit). The bridge makes the three-row grid exact for GBM value, which is why the oracle agrees — but it caps the window at two decisions, which is what the window-touch registration's oracle cannot resolve, and every CMC profile assertion in `test_partial_barrier_json.py` reads three rows. |
...
| ~~A declared default never reaches a deal's field dict, so a schema-valid block can price as zero silently~~ **CLOSED, narrowly and on purpose** | `schema.DealFields` × `Deal.__init__` | Taken 2026-09-02 with the APPLY-ON-LOAD remedy at one seam — `Deal.__init__` wraps the authored block in `schema.DealFields`, whose `__missing__` answers a read-by-name from the declaration — **restricted to an ALLOWLIST of exactly the fields whose declared default is genuinely the engine's fallback** (`schema.COMPLETABLE`: `Barrier_Monitoring_Frequency`, `Cash_Rebate` — the row's own shape). The blanket seam was built first and its review KILLED it with a measurement: a schema-valid block pricing as zero silently would have become a schema-INVALID block pricing at a PLAUSIBLE WRONG NUMBER silently — an `FXBarrierOption` omitting `Strike_Price` priced 741.53 where HEAD said `nan`, and omitting `Barrier_Type` priced the Down_And_**In** value, the product silently flipped — because the economically mandatory fields declare non-REQUIRED defaults (`0.0`, `'Down_And_In'`, `'Buy'`, `'Call'`). With the allowlist, every drop-one-field probe reproduces HEAD's behaviour except the two named repairs. THE SPLIT IS THE DESIGN: `get`, `in`, iteration and the JSON round trip see exactly the authored keys, so `plan_hash`, the factor universe and a saved book are byte-identical — measured over every job document in the tree, independently re-swept by the review from a pristine worktree (0 differing documents), and the audit that stopped the blanket's other half is recorded: of 148 declared `.field.get` sites, **36 disagree with their declaration**, three fatally (`config.py`'s two `is not None` discovery guards would mint garbage factors off a blank default; `calculation.py:2088`'s Reference stamp), so the calculation store's second half — hold a surviving fallback to its declaration — has NO deal-side twin, and those 36 are enumerated in `test_declared_defaults.py` for a later ruling. A default is CONVERTED, not copied (`schema.engine_default` — a Period through `Config`'s own grammar). The repaired shape, measured end to end: the omitted-field `FXBarrierOption` left NO row in `Results['mtm']`; it now prices 78.9325214257 / 6.3673278714 / 77.8327278324 / 8.6240679272 across the four spellings, each bit-identical to hand-furnishing. `structures.materialize`'s furnishing stays (redundant, not wrong). Gate: `tests/test_declared_defaults.py` (80), including `test_an_economic_field_is_not_completed_and_does_not_price`. |
| `StructuredDeal.Net_Cashflows` is declared with an underscore and read with a space | `instruments.StructuredDeal.post_process` | Found by the defaults audit (2026-09-02): the class declares `F('Net_Cashflows', default='Yes')` while `post_process` reads `field.get('Net Cashflows', 'No')` — the same words with a space. The declared key reaches no read, the read reaches no declaration, so a structured deal authoring `Net_Cashflows: 'Yes'` silently takes the un-netted path and the Workbench panel writes a key nobody reads. Reconciling flips the effective default for every `StructuredDeal` omitting it, so it needs its own measurement before its one-line fix. |
...
| ~~Analytic barrier/option arcs no test executes — the count is stale and the census can no longer take it~~ **RE-ANCHORED AND RE-MEASURED (2026-09-02); both named gaps CLOSED** | `pricing` (analytic barrier/option family) | TWO imports stopped the measurement, not one: the purged ledger (now INLINED in `gates/pricer_branch_census.py`, its AST half surviving as `--anchors`) and the tracer — `coverage`, in no requirements file and not installed on this box, so the census could never have run here at all. It is stdlib `sys.monitoring` now (PEP 669; a traced file runs at untraced speed) and runs standalone in a pinned worktree (arcs are line numbers — an edit mid-run re-attributes every arc below it, which killed the first attempt). Fresh reading: **59 unexecuted arcs at 1ed927a** — not comparable to 64/65 (different instrument, and a pricer that starts being CALLED trades one `never-called` line for its interior arcs); NOT yet re-taken over this wave's own diff (`partial_window_rebate` is added to `FAMILY` and traced clean, but the full ~2.5 h re-take has not run, so 59 is not pinned). **`pv_american_option` CLOSED** by `tests/test_american_option_json.py` — three worlds at live `r ≠ q`, both types, strikes 80/100/120, against a converged CRR binomial: Bjerksund–Stensland prices a SUB-OPTIMAL policy so the gate is a BOUND and it holds everywhere (Black ≤ engine ≤ binomial, worst gap 2.07%, premium capture 82.3–99.9%, the `q = 0` call BIT-IDENTICAL to its European, a call past its trigger marking exactly intrinsic). **The knock-OUT arms CLOSED** by `tests/test_barrier_arms_json.py` — all sixteen spellings of the eight `getbarrierpayoff` arms at rebate 0 and 40, against a longhand Reiner–Rubinstein (1.4e-14), a bridge MC (1.3e-2) and arm-by-arm in-out parity (7.2e-15); nothing was wrong, an arm slip dies by ≥8 orders. Correction recorded: `pv_barrier_option`'s KI rebate DOES carry pre-expiry value through its `E` term (41.65 vs 21.28) — the missing-rebate defect is the partial barrier's alone. Named leftovers: `pv_MC_Tarf`'s put arm has LEFT the census's scope (the flag moved into `oss_truncated_draw`, outside `FAMILY` — widening it is a one-line owner decision), and `coverage` remains an undeclared dev dependency of `gates/impacted.py --build-map`. Two of its named entries have since been struck by work elsewhere: `pv_partial_barrier_option` and `getpartialbarrierpayoff` are exercised by `test_partial_barrier_json.py` over all eight direction/window configurations (the row above), and the analytic knock-**IN** leg is reached now that `ForwardExtra`'s `reversion` leg declares an `Up_And_In` `FXBarrierOption` at continuous monitoring — `tests/test_structures.py` prices `Up_And_In`, `Down_And_In` and `Up_And_Out` through it, with `test_a_knock_in_plus_a_knock_out_is_the_vanilla` demanding a number from `if direction == BARRIER_IN:` to 1.1e-16 relative. What is still named and unstruck: `pv_american_option` (every equity fixture is European), and the knock-OUT formulas other than the Down-and-Out / Call / K > H arm every non-structure fixture takes — the LAST elif of the OUT block. `pv_MC_Tarf.bs_call_put_fwd` was dead rather than uncovered and has been **deleted**, with its ledger row. |
...
| ~~A spot model cannot be keyed off the pair's BASE currency, so a USDZAR TARF rides GBM on a calibrated book~~ **CLOSED 2026-09-02** | `utils.spot_model_currency` × `utils.hn_reciprocal_gamma` | Three keyings meet at an FX accrual leg and they do not agree. The ENGINE resolves a deal's spot-model parameters by naming convention off `Underlying_Currency`. The CALIBRATION (`fx_surface_block`) writes the pair's NON-DOMESTIC token, because an `FxRate` is priced in the domestic currency and that is the only leg of the pair the engine can simulate at all. And `furnish_accrual` forces a TARF onto the pair's BASE currency, because a target is a cap on a sum of DIFFERENCES and `1/S − 1/K` is not the reciprocal of `S − K`. So on a USD-base book a USDZAR TARF looks up `HestonNandiModelParameters.USD` while `POST /book/hn` wrote `.ZAR`, and it prices GBM however many times the pair is calibrated. EURUSD joins (its base IS the non-domestic token) and so does either side of an ACCUMULATOR, which has no target and crosses freely — `test_a_leg_quoted_under_a_model_books_into_a_book_that_marks_it` is the joining arm, priced under the model and booked with it. **Tonight's half is HONESTY**: the leg's absence note names the factor looked up, the factor the book actually carries, and the orientation that works, so a desk does not re-run a calibration that already ran (that gate is re-based by the closure below as `test_the_absence_note_names_the_factor_the_book_would_need` — on a calibrated book the TARF now PINS and the note is `None`). **CLOSED on the owner's 2026-09-02 ruling — one law per pair, on the engine's axis, and the READER learns the axis.** The single-token name already meant the engine-axis pair (`.ZAR` IS ZARUSD, everything against the base), so the calibration wrote the right block all along and only the readers moved: ONE rule, `utils.spot_model_currency`, at FOUR keyings — the engine's `get_spot_model_params_factor` callers, the runner's `structures.spot_model`, the approval's `pin_models` (same bug, would have refused a bookable quote), and `Config.calculate_dependencies`' own discovery, the fourth the contract did not name, which otherwise loads a block the compile will not ask for. The base currency is a NUMERAIRE and can name no block; a book that declares no `Base_Currency` REFUSES instead of pinning (the review's blocker: `structures` read only the explicit block and silently fell back to the pre-fix token — `spot_model_currency` now refuses `base is None` by name, and the rule is spelling-blind on `check_rate_name` tuples). Measured on the gate's book: the forced TARF's solved strike under the declared model was BIT-IDENTICAL to its GBM one (16.774620757621133 both ways, separation exactly 0.0 — the defect's signature); it now separates by **3.78%** against a 2.5e-5 solve floor, and the orientation that always joined is unmoved to the bit (18.15503015327775). **THE RULING'S SECOND HALF NEEDED ONE CORRECTION, MEASURED.** A base-underlying deal sits on the reciprocal of the fitted axis AND settles in the other currency, so the crossing is a change of NUMERAIRE as well as of axis: simulating the fitted law as-is and paying `1/S` leaves one variance of Siegel drift — the two orientations of one accumulator solve strikes **3.4e-3** apart and the gap does NOT close with paths (3.12e-3 → 3.42e-3 flat, 1k→64k). The `FxRate` is itself the density that changes numeraire, so the change shifts the innovation by exactly one standard deviation and `(ω, α, β, γ*)` for `s` describes `1/s` as `(ω, α, β, 1 − γ*)` at the deal's own carry — one law, two currencies, ONE DERIVED PARAMETER, never a second fit, so the F4 `(h, q)` handoff is untouched. Carried (`HN_Invert` in the kit; nothing in either pricer knows the axis — no reciprocals, no level inversion, no direction flip), the two orientations agree to **4.2e-6** at 16,384 paths, tighter than the lognormal's own 2.5e-5 floor; the map is pinned WHOLE in closed form against the FX option symmetry through `hn_call`'s own Fourier inversion at **1.4e-12**, where the sign flip alone misses by 3.2e-2 and the uncarried parameter by 5.2. **NOT TAKEN, a scope reduction rather than a deferral**: the COMPONENT family does not transport — the change puts a state-dependent term in its long-run intercept (`ω_t + φ(1 − 2γ₂)h_t`) and leaves the family — so a component deal whose underlying IS the base REFUSES by name (`UnpriceableSchedule`, FATAL), crisp arm included, naming the plain family and the other orientation as remedies; the order −1 partial moment reserved for the smooth machinery is NOT built and not needed (the deal-axis walk is an ordinary plain-HN walk, `Branch_And_Weight` meets the plain family's existing refusal, `HN_Stride` is component-only and never reaches an inverted deal). A CROSS pair keeps `Underlying_Currency` byte for byte, as ruled. **FOUND AND FIXED ALONGSIDE**: `pv_MC_Tarf`'s remaining target could run NEGATIVE and drive the PnL cap through a `log` of a negative — `intr` clamps at the BLOCK's remaining target and only a truncated draw makes that the path's own; HN saturates a far cap (NaN on a plain EUR-underlying HN TARF whose GBM twin prices 99.69757214221812), and the fix pass then proved the GBM claim was fixture-true only: TWO observed-but-unsettled fixings inside one settlement lag drive `R` negative under plain GBM with no HN anywhere (199.956555282538 clamped against 102.011647350498 unclamped, closed-form oracle 199.96042 at a 1.5e-4 survival bound) — so `R = relu(R − accr)` clamps the observed-fixing branch of BOTH arms, crisp and smooth bit-equal, and every existing fixture is bit-identical. Gates: `test_structures.py::test_a_tarf_on_a_fitted_pair_stops_riding_gbm`, `::test_the_accumulator_solves_one_strike_from_either_axis_under_the_model`, `::test_a_book_that_declares_no_base_currency_refuses_instead_of_pinning`, `::test_the_token_rule_answers_the_same_token_in_either_spelling`, `::test_the_absence_note_names_the_factor_the_book_would_need`; `test_fx_accumulator_json.py::test_a_base_currency_underlying_prices_on_its_own_axis_under_the_fitted_law`, `::test_one_trade_authored_from_either_axis_is_one_number_under_the_fitted_law`, `::test_a_family_that_cannot_be_carried_to_the_reciprocal_axis_refuses_by_name`, `::test_a_cross_pair_keeps_the_underlyings_own_read`; `test_fx_tarf_json.py::test_two_observed_fixings_in_one_settlement_lag_cannot_drive_the_target_negative`; `test_hn_component.py::test_the_reciprocal_carry_is_the_fx_option_symmetry_in_closed_form`. **This is the unblock for the Honest-HN-CVA program's step (1) on the FX book.** |
| The TARF's observed-fixing branch neither fires redemption nor caps per path | `pv_MC_Tarf` (`use_past_fixing` arm) | Surfaced by the keying landing's review (2026-09-02), measured under plain GBM with no Heston–Nandi anywhere, and left UN-BLESSED by that landing on purpose — both halves pre-date it. (1) On an OBSERVED fixing the alive weight `L` is never zeroed, so a deal whose banked accruals already exceed the target lands at `R = 0` without firing redemption and keeps paying its OTM leg. (2) Two observed-but-unsettled fixings each accrue against the BLOCK's remaining target rather than the path's own running `R`, so each can bank more than half the remainder — the new clamp stops `R` running negative (oracle-gated), but the double accrual itself stands and is a redemption-convention question: what does a TARF pay on the fixing that crosses its target when the crossing is only visible in settled arithmetic later? Needs an owner ruling on the observed-fixing redemption convention; the reading gate's docstring names both halves as un-blessed. |
...
| ~~The HN family's `Volatility` is REQUIRED but never read under `Quote_Type` Premium, and a missing reference SKIPS instead of refusing~~ **CLOSED** | `HestonNandiModelParameters` bootstrap | Closed 2026-09-01 by the equity fit-side landing: the quote type declares which references it reads — `Volatility` is resolved only under `Implied_Volatility` — and a missing REQUIRED reference now refuses by name to the caller (`Config.bootstrap` does not wrap it), never a `skipping` log line with no factor written. The circularity is measured shut: a chain-sourced `Premium` block fits end-to-end in a book carrying NO `EquityPriceVol` at all, ATM residual 4.441e-16. Every existing FX fit byte-identical (surface, both ladders, all five fitted parameters, hexed old-vs-new). Gates in `tests/test_equity_chain.py`. |
| ~~One `Discount_Rate` does two jobs in the HN fit, and the pricer's equity forward reads a third curve~~ **CLOSED** | `HestonNandiModelParameters` bootstrap × `utils.calc_eq_forward` | Closed 2026-09-01: the block declares `Funding_Rate` (and the dividend reference) in the fields `resolve` reads, and the fit builds the forward the pricer builds — worst relative miss **0.000e+00 at every pillar on a 125bp repo spread**, where the undeclared-funding world misses by up to 3.68%. Declared defaults reproduce today's arithmetic exactly (spread-free world byte-identical). REFINEMENT CARRIED, emitter-side: `equity_chain.forward.rate` is one number feeding parity, strike placement and vega — with funding declared it must be the funding rate for strikes to land where the fit rebuilds them, so the chain-implied dividend and the cross-expiry vega weights carry the spread where `dividend_yield` is undeclared; declare both on a spread market. |
...
| The boundary estimator's bandwidth plateau is MEASURED and holds — CARRIED, not closed, because the documented operating point cannot run | `pricing.stochastic_boundary_correction` (`gates/boundary_bandwidth_plateau.py`) | Measured 2026-09-02 at 16384 paths (three seeds) and 20480 (the true ceiling): **the estimate does hold still over 0.005–0.08 — a factor of 16, the declared 0.01 one rung inside its lower edge** — seed-MEAN correction spread 2.41% (discrete barrier) / 3.87% (HN) over the factor-8 window, the reported CVA delta 0.60% / 0.24%, against seed floors of 13.69% / 28.52% — the bandwidth dependence is an order under the noise it must be told from, and NO single seed can see it (per-seed spreads 12.7–23.1% with seeds disagreeing on the drift's SIGN; the seed floor is also a floor on PART of the noise only — the Sobol stream is not derived from `Random_Seed`). The plateau has a lower edge (0.0025 reads 9–20% low with per-rung seed spread to 101% — kernel starvation), and the PATH COUNT is what buys it: at 2048 the correction falls monotonically 23.76% and nothing holds still, which is the row's old 512/1024 reading reproduced. The suppression seam is the DECLARED `Boundary_AAD_Bandwidth` at 1e-12 (empty-kernel branch, bit-identical to deleting `boundary_correction`, validated against this row's own recorded mutant digits). CARRIED because the acceptance names 32768 paths and that count does not run at all — the row below. (The two gates that used to pass with their subject deleted are re-baselined onto the declared grid; the HN barrier gate is now 1.18% against a 6.19% suppression mutant, and the discrete-barrier profile gate is replaced by a bit-exact rebate ledger.) |
| ~~Boundary scoping is not mutation-gated~~ **The CORRECTION is now mutation-gated (2026-09-02); its SCOPING specifically still is not** | `tests/test_boundary_scoping_dominance.py` | The portfolio where the correction dominates is AUTHORED, and the DEAL is what does it: a discretely monitored down-and-out DIGITAL struck at ~zero is worth `Cash_Payoff × P(never crossed)`, so its spot delta is the barrier flux and almost nothing else — correction/smooth **3.04** against 0.56 for the knock-out call the old gates use. Two collateralised sets at different barriers, both parked on the relu kink. At 16384 paths the correction is **2.96× the whole smooth sensitivity** (75% of the reported number, against 2.4% on the old fixture); suppressing it through the DECLARED `Boundary_AAD_Bandwidth` (1e-12, bit-identical to deletion, no monkeypatching) reads **381.77% off the CRN oracle — killed 12× clear** of a 30% tolerance that is the estimator's own residual (21.5% live at 2.83% flatness; falls with paths), where the identical mutant on the old fixture survives at 0.23%. THE RESIDUAL: a MIS-SCOPING mutant (set-level `portfolio_delta`, scoring off a zero portfolio) has NO public seam — every registration names its class directly — so scoping-specific mutation stays open, said in the gate's own docstring. |
| The boundary correction is wrong on a portfolio whose CVA relu never binds | `pricing.boundary_correction` × `utils.MTABoundarySet.objective_jumps` | Found by the correction-dominated fixture (2026-09-02). Lift a collateralised book clear of the relu with a DELTA-FREE cash cushion (a forward bought at strike zero plus one sold at K — worth exactly K in every scenario, no equity delta) and the objective goes locally linear while the counterfactual jumps stop being truncated: CRN disagreement **10.1 / 56.8 / 84.3 / 93.4 / 93.4%** at cushion 0/10/30/100/300, saturating exactly where `mtm.min()` reaches 0 — cushion 0 is a second netting set worth exactly zero and reads 10.1%, so it is the LIFT and not the scoping, which is the control. With a binding MTA the `MTABoundarySet` term alone carries **+0.016552 of a +0.017102 correction — 26× the whole true derivative** (AAD +0.0151373 against CRN +0.00063617), and the latched half is wrong the OTHER way (−4.79e-05 where the oracle wants +1.92e-04). Un-cushioned the same fixtures read 6.3% / 5.6% — fine. Nothing done; no gate ships against it, because a gate there would pin the defect. |
| A Monte Carlo pricer spends the PATH COUNT on Sobol's dimension, so `Batch_Size` above 21201 silently skips the deal | `pricing.oss_uniforms` × `calculation.CMC_State.quasi_rng` | Found by the bandwidth-plateau measurement, which could not reach its own documented operating point. `oss_uniforms` draws `quasi_rng(simulation_batch, n_fix * num_sims)` — the outer path count as the Sobol DIMENSION and the inner sims as the sample size, the TRANSPOSE of the convention `stochasticprocess.py` states for itself. `SobolEngine` caps at dimension 21201, so a larger batch refuses INSIDE the pricer, `Deal.calculate` swallows it into a `CRITICAL … skipped` line, and the run dies three layers downstream on the collapsed frame (`ValueError: Shape of passed values is (1, 1), indices imply (17, 1)`) — a refusal reaching the caller as a shape error. MEASURED: 20480 runs, 21248 does not, 32768 does not, independent of `MCMC_Simulations`. Every OSS pricer is on this path, while `Boundary_AAD_Bandwidth`'s own field note documents 32768 as the count its default needs — the engine documents an operating point it cannot run. `Simulation_Batches` does not rescue the estimator (`boundary_sets` clears per batch). The remedy is the axis swap the rest of the engine already uses (dimension = fixings × inner sims, samples = paths), which re-baselines every OSS fixture — hence a row, not a patch. |
| ~~The HW2F calibration prices every market premium LOGNORMAL and reads neither `Distribution_Type` nor the declared `Shift`~~ **CLOSED** | `bootstrappers.create_market_swaps` × `riskfactors.InterestYieldVol` | Found by the rates-emitter build (0de7f12). `create_market_swaps` priced with `utils.black_european_option_price` carrying only `vol_surface.BlackScholesDisplacedShiftValue` — whose property consults ONLY the undeclared `Property_Aliases` and otherwise returns 0.0, never the schema-declared `Shift` — while the DEAL path's `Factor3D.get_subtype` carries `(Distribution_Type, Shift)` into every swaption deal's `Volatility` dependency. So both the distribution AND the displacement diverged between the deal path and the calibration path: a NORMAL-vol ladder (the seeded ZAR `SASN` grid) fitted under a lognormal convention, and a block authoring `Shift: 3.0` calibrated at zero displacement, silently. **Closed by owner ruling 2026-09-01.** The premium construction reads `get_subtype()` — the DEAL path's own read, so there is ONE spelling of what a surface declares — and picks a matched `(numpy pricer, tensor twin)` pair out of `PREMIUM_CONVENTIONS`: `'Lognormal'` (the field's declared default, so every existing document is bit-identical) strikes Black at `K + shift`, `'Normal'` strikes the general-form Bachelier `A[(F−K)Φ(d) + σ_N√T·φ(d)]` off an absolute normal vol. **MEASURED, one numeric ladder read both ways on the four-quote fixture: the premiums are 11.374x / 9.843x / 9.683x / 10.926x apart** — 1/F to a part in 1e5, since the ratio of an ATM Bachelier premium to an ATM Black one is `σ√(T/2π) / (K(2Φ(σ√T/2)−1))` — which is what the defect cost on every benchmark. A Normal four-quote block now solves in 11.5 s to `||J'r||` **3.97e-7** against `||r||` **1.75e-6**, three orders inside the declared `Stationarity_Tol`, and the market side ROUND-TRIPS: a quoted σ_N through the Bachelier premium through the closed-form inversion comes back as itself to **0.0–4.4e-16** — but times **0.99965771007041049**, which is `√(T_365.25/T_curve)`, the premium's 365.25 clock against the inversion's ACT/365 one. That clock is **CLOSED (owner ruling 2026-09-02, landed beside the α-seed fix as ONE re-marking event): THE CURVE'S DAY COUNT WON.** `create_market_swaps` builds `expiry` once per benchmark as `curve_factor.get_day_count_accrual`, and that one float prices the numpy premium, strikes the float64 twin, brackets the `Volatility_Delta` `brentq`, and is `swaption_schedule_class.expiry` — one spelling, the read the fit already did. The round trip returns σ_N as ITSELF (0.0–2.2e-16, was ×0.99965771) and `∂r/∂q` under `'Normal'` is exactly −w. It is NOT a Normal-path repair: the same clock moved every LOGNORMAL premium by +3.31e-4 to +3.41e-4, which is why it re-marks every θ\*; measured at a pinned θ, the Schrager–Pelsser side is bit-identical on all 25 benchmarks and only the market premium moves — one clock, one side of the residual. Schedule DATES are untouched: `exp_days`, the grid search and both leg generators read days and the instruments' own declared day counts. The displacement half is `InterestYieldVol.displacement`: declared `Shift` first, `Property_Aliases` the documented legacy behind it, a premiums file's own column under that — the two routes reaching the strike through the *same* division (`Percent(2.0).amount` **is** `2.0/100.0`) and therefore bit-identical, gated in all four corners as hex. A **zero** `Shift` is not an instruction (it is the field's own default, so an authored zero cannot shadow a live alias — which is what keeps every existing file bit-identical), and a displacement authored beside `'Normal'` from *either* spelling refuses by name. Bit-identity held to the hex digit: `MC_FOUR_THETA`, `AN_FOUR_THETA` and the 25-quote readings (‖J'r‖ 3.85e-6, ‖r‖ 2.26e-3) all unmoved. Gates: `test_a_normal_surface_calibrates_and_the_market_side_round_trips`, `test_the_two_conventions_are_two_prices_and_the_normal_one_is_the_bachelier_premium`, `test_a_normal_block_carries_the_quote_side_and_its_bachelier_derivative`, `test_a_declared_shift_reaches_the_premium_and_refuses_beside_a_normal_quote`, and the rewritten `tests/test_swaption_vol_emitter.py::test_the_engine_reads_the_declared_distribution_and_this_block_says_which`. **The declaration lives on the SURFACE, which the Bloomberg emitter does not author** — a desk pointing `Swaption_Volatility` at a lognormally-declared factor still gets a lognormal fit of normal quotes, and the block's `Quote_Source` line is where that is said. |
...
| ~~The `Volatility_Delta` re-strike bracket assumes a lognormal scale, so a NORMAL ladder below ~78bp cannot be re-struck~~ **CLOSED** | `bootstrappers.create_market_swaps` (the two `brentq` re-solve sites) | Closed by owner ruling 2026-09-01. `IMPLIED_VOL_BRACKETS` is co-keyed with `PREMIUM_CONVENTIONS` and read at the SAME seam off the same declared `Distribution_Type`: `Lognormal` keeps the historical `(0.01, vol + .5)` character for character, `Normal` brackets MULTIPLICATIVELY around the row's own quote (a hundredth under, a hundred over, the quote floored at 1e-6) — right because the ATM Bachelier premium is linear in the vol, so the implied vol is a division and the bracket has only to CONTAIN it. THE FALLBACK COULD NEVER HAVE RESCUED IT: at F = X the Bachelier premium does not mention the strike, so the re-solve was the SAME function and a normal ladder under the floor was fatal, not degraded. MEASURED: 145–118bp brackets, flat 100bp brackets, 78bp and 40bp refused at f(0.01) of +7.7e-04..+2.0e-02 (a sign failure, three to five orders above noise, reproduced in-process); the re-strike now round-trips as an identity to 0.0–3.3e-16 on all four benchmarks, and the Lognormal arm is hex-identical on eight re-struck premiums measured in a pristine worktree. Gate: `test_the_normal_re_strike_brackets_in_its_own_scale_and_the_lognormal_arm_is_unmoved`, whose own fixture refuses at 91c29de. |
...
*~~The cornered θ\* is a SEED defect~~ — CLOSED (2026-09-02): `ALPHA_SEED = (0.5, 0.05)` is
declared and the corner is dead.* The 2026-09-01 probe stands as the diagnosis (θ\* on the ρ
bound with six floored σ knots, 100.00% of ‖J′r‖ bound multipliers, `Stationarity_Tol` passed
by 48×, a separated seed reaching 0.427 bp against 1.348 on the live ladder) — with ONE
mechanism correction the landing measured: the multiplicative basin step DOES separate an equal
pair (one multiplier per coordinate; median |log-ratio| 0.477 over 2000 walks). What the
symmetric seed cost was basin hopping's ITERATION-0 DESCENT — scipy minimises at x0, and at an
exchange-symmetric point the gradient carries the same number in both coordinates: the old
seed's first descent lands α₁=α₂=0.0118 at loss 8.95e-6, the declared seed's lands
(0.500, 0.0027) at 5.33e-6. On the identified fixture: rms 4.5125 → **2.9594 bp**, ZERO σ knots
on a bound (was a floored knot plus one 7e-7 off the ceiling), and EITHER half of the landing
alone kills the corner (the retired seed on the clock-fixed engine also reads 2.959). What
remains at a bound is the FIXTURE'S OWN MINIMUM, not a corner: ρ runs to −0.95 for every seed
reaching the good basin, the objective's profile along ρ is MONOTONE to the bound, and the
PROJECTED gradient — the reading that means constrained stationarity — is 2.09e-7 against the
retired vector's 4.47e-5 (214×), gated with the retired vector carried as the non-vacuous
contrast. ONE READING WENT THE OTHER WAY, gated rather than absorbed (owner's eye wanted): the
honesty reprice's WORST benchmark reads −6.25% against −4.64% while its rms improved
2.71% → 2.39% and the outside-3% count fell 10 → 3 — the max is one order statistic,
anti-correlated with the fit on this flat-quoted cube (the seed landing in the WORSE basin
reads the best max; "rms strictly lower" and "worst no worse" are jointly unattainable here),
and the live ladder's auditor moved the right way in the probe (−3.39% → −2.12%). The live-fit
readings above (rms 1.348 bp, honesty −3.39%, recovered/quoted 0.999657710) are the 2026-09-01
run's, under the retired seed and clock — the next live fit re-marks them and the round-trip
ratio is now 1.0. The two emitter design questions are
...
**Two-sided WINGS and the FVA column are BUILT (2026-09-02).** The quote layer consumes the
RR/BF rows' own two-ways — each side's copy of the surface SKEWS by the wing halves at their
pillars through the same strangle algebra it was built from, the ATM flat shift unchanged, a
wing-less book byte-identical (measured old-module-vs-new on four books), and the reported
`wing_spread` is the half the quote actually dealt at (scaled by the risk policy's own
`scale`). And the XVA mosaic gains its first later column: `fva` beside `cva` off one run per
set (`Funding_Valuation_Adjustment` in the same job), old rows tolerated as staleness, a live
run missing the table filing `failed` rather than a null. CARRIED FOR THE OWNER: turning FVA on
scales the set's MTM by survival (`calculation.py`'s own comment says the scaling is for FVA
while it reaches the CVA integrand too), so the shipped `cva` column moves −0.97%/−2.38% on the
gate's sets when FVA joins the job — real at source, gated by nothing absolute, and a ruling on
whether one run may serve both or the mosaic runs FVA separately.

**BARRIER STATE IS A FOLD OVER FIXINGS — ratified 2026-09-02; builds beside spine increment 4,
whose hydration it belongs to.** No deal stores a consequence: `Barrier_Hit` retires from the
accumulator's schema (and from `structures.py`'s leg furnishing — it is already redundant there,
OR'd with the fold's own `settled_breach`), a document carrying it refuses as consequence-shaped
— the spine vocabulary's "knocked_out is a projection" applied at the deal schema — and hit-state
is DERIVED at compile from the deal's terms plus observed fixings: any observed fixing on a
monitoring date ≤ base date crossing the declared level, per the declared direction. The fold's
output splits on ONE criterion — does the deal still take future decisions? NONE LEFT: the deal
compiles AS what it became (a hit KO is its rebate cashflow at its declared timing, a hit KI IS
the vanilla, a touched one-touch a fixed cashflow, a called autocall its coupon at that fixing's
settlement) — the `hit_value` history is the argument for transformation over a pricer branch:
+1432% grew in a half-dead already-hit branch nobody read. DECISIONS REMAIN: same deal, folded
parameters (the TARF's remaining target, the accumulator's `pending` — both already this shape).
Two refusals, both fail-loud: a monitoring date ≤ base date with NO observed fixing refuses by
name (absence of a fixing is not absence of a hit — assuming alive is the silent wrongness the
fold exists to kill), and the consequence field refuses on input. DISCRETE monitoring reads a
close per `Barrier_Dates` row — the table grows an `Observed` column (the accrual deals' untagged
row shape; the interim home until hydration). CONTINUOUS monitoring reads DAILY BARS — (low,
high) per business day under `(index, date, source)`, the owner's ruling: a bar is a FACT (the
exchange's own extremes; `PX_HIGH`/`PX_LOW` on the testbed), it brackets every intraday print so
bar-vs-level IS "traded through the level that day" per that source, and a disputed determination
is a SUPERSEDED BAR, never an edited flag. The tick loop may post the running session bar under
its own source token, superseded by the settled bar — marks stay current intraday, the record
stays fact-only. The fixing INDEX is derived from the deal's own underlying factor name, never
declared (a second spelling is the sibling-fallback defect class); only the SOURCE is declarable
(`Fixing_Source`, blank default = the book's ticked source), joining the supersession key — the
float legs' `Reference` threading pattern. The symmetry the design keeps: past extremes are
RECORDED (bars), future extremes are MODELLED (the bridge crossing probability) — one design,
two sides of base date. A t0-folded state registers no boundary set (death is data, not simulated
state — the accumulator's own rule); only in-simulation decisions register, so the sensitivity
architecture is untouched. Buildable now: the Observed column, the fold, the transformations, the
refusals, the bar predicate. Increment 4's: hydrating both fact kinds from the log, where the
book file becomes a projection.
...
`‖J'r‖` **3.16e2** (`‖r‖` 22.11; 8.24e3/45.97 before the 2026-09-02 seed+clock re-mark), down from
2.86e11 at the seed — about nine of eleven orders
— which is why the gate file has to declare `Stationarity_Tol` **1e5**; the analytic chain lands at
**8.63e-7** (`‖r‖` 1.48e-3), *inside the field's own 1e-3 default*. Eight orders of declared
...
everywhere. **(2) Stationarity**, and this is the one that decides it: `‖J'r‖` at θ\* is **8.63e-7**
on the plain residual against **3.16e2** on the quartic (3.85e-6 / 8.24e3 at the ruling; the
2026-09-02 seed+clock re-mark moved both, the relation unchanged) — *inside* `Stationarity_Tol`'s
own **1e-3** declared default rather than five orders outside it. A family whose default objective cannot clear
...
**Standing consequence, the second (2026-09-02 seed + clock).** Every HW2F θ\* solved before
2026-09-02 re-marks — the second such event, after the domestic-measure fix. Both halves reach
every block: the `ALPHA_SEED` moves where the chain starts on any block without an existing
parameter factor, and the premium clock moves the market side of BOTH objectives on every block,
lognormal and normal alike (+3.31e-4 to +3.41e-4 per premium). A desk naming an old θ\* has two
options and no third — re-baseline, or re-solve. Carrying an old θ\* forward is a third thing
that LOOKS like the first: a block still holding its parameter factor warm-starts off it, so the
fit moves but the seed change does not reach it, and the answer is neither the old mark nor what
a fresh block gets. Emissions that move with it: `Quanto_FX_Correlation_1/2` off `save_params`
(which split their sign on the repo fixture), and every locus and diagonal recorded downstream
of a fit.
...
the textbook `O(‖r‖)` at **1.50e-4** of `J'J` (8.75e-4 at the old θ\* — it shrank with the fit,
which is the O(‖r‖) claim coming true), against the Monte Carlo residual's **0.500064**.
...
the Monte Carlo chain stopped at 8.24e3 (3.16e2 since the 2026-09-02 re-mark) — and the ladder is the Monte Carlo path's ladder to two
...
the real component-HN bootstrap (ATM residual 4.4e-16). THE THREE ENGINE FINDINGS ARE CLOSED (Known defects, rows above): the quote type now
declares which references it reads and a missing one refuses by name instead of skipping, so a
chain-sourced `Premium` block fits in a book carrying no surface at all — measured end-to-end,
ATM residual 4.441e-16 with no `EquityPriceVol` in the world; the block declares a
`Funding_Rate` and the fit's forward is the pricer's to 0.000e+00 at every pillar on a 125bp
repo spread, with every existing FX fit byte-identical; and the option row declares the two-way
and the stamp, so `European_Options` is a value-plane table and a re-quoted chain ticks rather
than re-authors. The
