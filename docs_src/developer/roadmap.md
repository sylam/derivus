...
*The cornered θ\* is a SEED defect, measured (2026-09-01).* θ\* comes back with ρ exactly on
+0.95, six of twenty σ knots on the 1e-5 floor and α₂ negative — and none of that is the
ladder's preference. Under the DECLARED box, a local least-squares seeded at a separated pair of
reversion speeds reaches **0.426954 bp** rms against θ\*'s 1.348176, at α = (+0.8235, −0.0053),
ρ = −0.6387, with one knot near a bound instead of eight, ‖J′r‖ 2.9e-8 instead of 2.1e-5, and
the engine's own Monte Carlo auditor at −2.12% instead of −3.39%. Widening ρ's bound is worth
−0.000559 bp and re-corners at the new bound; dropping the σ floor to 1e-8 is worth −0.000491 bp.
The cause: `implied_process` seeds `Alpha_1 = Alpha_2 = 0.1` with two identical σ curves — two
identical factors the multiplicative basin step never separates — and the corner is the
collinearity signature (loading cos-angle 0.995–0.998 at θ\*'s speeds). θ\* also breaks the
quote-side contract silently: 100.00% of its ‖J′r‖ is bound multipliers, `Stationarity_Tol`
passes it by 48×, and the GN contraction is off 4.21× on the movable coordinates; a re-seeded
fit re-marks the emitted quanto correlations from (−0.397, −0.392) to (−0.320, +0.020). THE
REMEDY IS A SEED, not a bound: separate the α seeds (a build to take deliberately — every HW2F
θ\* re-marks). The two emitter design questions are
