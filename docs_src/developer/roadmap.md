...
- **`pv_MC_Tarf` (`accumulation`)** — The opening accrual nets every declared reset, settled or not, while the loop pays the unsettled ones again out of the remainder that netting left — a known-but-unsettled fixing is counted on both sides and banks the remaining target rather than its own accrual. The index is also offset by one per declared reset, so the dense redemption chain's flags and the strip's crossings agree only where the schedule declares no resets (the `recon_max` debug line states it per run). Found closing the redemption convention. *Measured:* on the lagged fixture two 0.2 accruals leave the remainder at 0.1 and the crossing fixing banks 0.1 where the fixing is worth 0.2. Also untested: a collateralised set reading a knock-out rebate's 14 declared settlement dates.
