...
| A barrier date whose coupon row is ZERO reads the previous fixing's spot, and mis-tenors the coupon after it | `pv_MC_AutoCallSwap` (no-averaging loop) | Found by review of the branch-and-weight landing, on a document the fixture set did not have: coupons 0.08 / **0.00** / 0.08 at days 180 / 270 / 365, one price fixing per date, thresholds 1.0, and the deal's only put barrier (0.7) dated on the ZERO row. `calc_dependencies` puts that on the fast `no_averaging` arm — `ac_dates` counts the zero row and day 270 IS a coupon date, so neither the fixing count nor the barrier-alignment test objects — and then `if coup > 0` is FALSE at the barrier's own `j`: the coupon block never runs, so `Sj` is still the day-180 draw when the breach indicator reads it, and `coupon_index` never advances, so the 365-day coupon takes the price-fixing strip's 180→270 interval (**90 days**) in place of its own 185. Two readings of one un-run block, and they compound. MEASURED: **+0.3948092082267596** at 2^19 paths, byte-identical absent / `'No'` / `'Yes'`. An independent trapezoid reference written from the DEAL to reproduce the code as written reads +0.395432 (0.16%, estimator noise); the same reference reading the barrier at its OWN date over the real 95-day tail reads **+0.317939 — 24.2% away**. **THE SWITCH IS NOT WHAT DOES THIS**: no interval is built at that `j`, so both estimators take the same crisp indicator and off-is-off holds here to the bit — the defect is the arm's and predates the landing. REMEDY: at a barrier row whose coupon is `<= 0` but which carries a price fixing, advance `Sj` over that row's own interval — UNtruncated, since there is no autocall decision to survive there — and advance `coupon_index` with it; the alternative is to refuse such a row by name in `calc_dependencies`, which retires the document rather than pricing it. Either one moves a crisp number, so it is a ruling and not a patch. Pinned meanwhile as a READING with its mutant named: `test_branch_and_weight.py::test_a_zero_coupon_barrier_date_reads_a_stale_spot`, which goes red the day the defect is fixed — which is when this row closes. |
...
*What that licensed, and what it cost.* `Objective` is a declared field — `Monte_Carlo` by default
and bit-identical to what this family always did, `Analytic` differencing **vols against vols,
plain**. The residual is `Weight × (σ_SP(θ) − σ_market)` with the market side a CLOSED-FORM
...
gradient, and a chain whose two stages minimise the same function. **Two things it owes.**
`Quote_Sensitivity` on the analytic path REFUSES by name — the market vol enters that residual
linearly through the inversion, so the quote side there is nearly trivial and deserves its own
gated build rather than riding this one; the refusal names `Monte_Carlo` as today's remedy and
this row is where the next step lives. And the analytic solve ends with an **honesty reprice**: one
...
nowhere else. **The default does not flip** on this measurement: that is a ruling, not a build.
