...
`DV_MCP`/`DV_Bloomberg` are console scripts, and `desk` is the one-line install.
...
deals from the UI. The structures registry's named next steps: the RISK-IMPACT step (book plus
candidate under `Greeks`, the skew delta in RR/BF coordinates pricing the spread — every hedge
layer now speaks tradable coordinates), barrier legs for the forward-extra family (the pricers
exist; the registry needs to name a barrier leg), and a ratio-solve primitive for participating
forwards.
