...
| The autocall's settle by-product books neither `nominal` nor its direction, per coupon | `pv_MC_AutoCallSwap` (no-averaging loop, the `tau == 0.0` settle) | Found by the OSS seam census that fixed the TARF's twin (`test_tarf_cash_settle.py`; the TARF booked its fixing unsigned, so a SOLD one settled a receipt — one line, gated). The autocall's is worse two ways and unfixed: it settles `P.mean` with no `nominal` at all (units AND Buy/Sell missing), and the `tau == 0.0` append sits INSIDE the coupon loop while `tau` is row-level, so a settling row appends the running PV once per remaining coupon and `cash_settle` sums the partial accumulations. Needs its own investigation of what the ledger SHOULD book (the settling coupon, undiscounted, once) before any fix — every current fixture is Buy and none asserts the autocall's cashflow ledger. |
...
| A fixing dated ON the base date silently discards its declared value | `utils.make_fixing_data`, consumed by `pv_MC_Tarf` and `pv_MC_Accumulator` | `make_fixing_data` writes the observed value only when `fixing[0] < reference_date`, so a fixing dated exactly today records `0.0`, and `get_scenario_offset` gives it `Scenario = 0`, which routes it to the SIMULATED samples. The pricer then uses the simulated t0 spot in place of the number the trade actually fixed at. Measured on an accumulator declaring 1.30 for a day-0 fixing: `Price_Fixings` reads `[1.12, 0., 0.]` and the mark carries no 1.30 contribution. Inherited by the accumulator from the TARF rather than introduced — both are exposed, and a same-day fixing is the ordinary state of a deal on its own fixing date. Wants a decision on whether today's declared fix beats today's simulated spot (it should) before a fix. |
