...
| --- | --- | --- |
| `pricing.oss_uniforms` × `calculation.CMC_State.quasi_rng` | `oss_uniforms` draws `quasi_rng(simulation_batch, n_fix * num_sims)` — the path count as the Sobol *dimension*, the transpose of the convention the rest of the engine states. `SobolEngine` caps at dimension 21201, so a larger batch refuses inside the pricer, `Deal.calculate` swallows it into a `CRITICAL … skipped` line, and the run dies downstream on the collapsed frame. Remedy is the axis swap (dimension = fixings × inner sims), which re-baselines every OSS fixture. | 20480 runs; 21248 and 32768 do not, independent of `MCMC_Simulations`. Every OSS pricer is on this path, and `Boundary_AAD_Bandwidth`'s own field note documents 32768 as the count its default needs. |
| `pricing.boundary_correction` × `utils.MTABoundarySet.objective_jumps` | Wrong on a collateralised portfolio lifted clear of the CVA relu by a delta-free cash cushion: the objective goes locally linear and the counterfactual jumps stop being truncated. No gate ships — a gate there would pin the defect. | CRN disagreement 10.1 / 56.8 / 84.3 / 93.4 / 93.4% at cushion 0 / 10 / 30 / 100 / 300, saturating where `mtm.min()` reaches 0. The control that separates lift from scoping: cushion 0 is a second netting set worth exactly zero and still reads 10.1%, so it is the lift. With a binding MTA the `MTABoundarySet` term alone carries +0.016552 of a +0.017102 correction — AAD +0.0151373 against CRN +0.00063617, 26× the true derivative — and the latched half is wrong the other way, −4.79e-05 where the oracle wants +1.92e-04. Un-cushioned: 6.3% / 5.6%. |
| `pricing.stochastic_boundary_correction` (`gates/boundary_bandwidth_plateau.py`) | The bandwidth plateau holds and is carried, not closed, because the documented operating point cannot run (the Sobol row above). The declared `Boundary_AAD_Bandwidth` default 0.01 sits one rung inside the plateau's lower edge. The suppression seam is that same field at 1e-12, bit-identical to deleting the correction. | At 16384 and 20480 paths the estimate holds over 0.005–0.08: seed-mean correction spread 2.41% (discrete barrier) / 3.87% (HN), reported CVA delta 0.60% / 0.24%, against seed floors 13.69% / 28.52%. No single seed sees it — per-seed spreads 12.7–23.1%, seeds disagreeing on the drift's sign, and the seed floor bounds part of the noise only because the Sobol stream is not derived from `Random_Seed`. Lower edge at 0.0025: 9–20% low, per-rung seed spread to 101% (kernel starvation). At 2048 paths the correction falls monotonically 23.76% and nothing holds still. Acceptance names 32768. Re-baselined onto the declared grid: the HN barrier gate is 1.18% against a 6.19% suppression mutant, and the discrete-barrier profile gate is a bit-exact rebate ledger. |
| `tests/test_boundary_scoping_dominance.py` | The correction is mutation-gated; its *scoping* is not — a mis-scoping mutant (set-level `portfolio_delta`) has no public seam, since every registration names its class directly. Said in the gate's own docstring. | The fixture is authored because correction/smooth is 3.04 on a down-and-out digital struck at ~zero against 0.56 for the knock-out call the old gates use. At 16384 paths the correction is 2.96× the whole smooth sensitivity; suppressed through the declared bandwidth it reads 381.77% off the CRN oracle, 12× clear of a 30% tolerance that is the estimator's own residual (21.5% live at 2.83% flatness, falling with paths). The same mutant on the old fixture survives at 0.23%. |
...
| `pricing.sim_spot_oss`, `pricing.sim_spot` | A reporting row that *is* an observation date opens with `dt = 0`, and that step is simulated at 1% of lognormal vol instead of being resolved by an exact indicator. It consumes a Sobol draw, so it moves every barrier constant. | Immaterial for a down-and-out call; material for a digital or a cash rebate. The `no_averaging` branch already resolves `dt <= 0` exactly. |
| `pv_MC_Tarf` (`use_past_fixing` arm) | Two halves, both pre-existing. (1) On an observed fixing the alive weight `L` is never zeroed, so a deal whose banked accruals already exceed the target lands at `R = 0` without firing redemption and keeps paying its OTM leg. (2) Two observed-but-unsettled fixings each accrue against the *block's* remaining target rather than the path's own running `R`, so each can bank more than half the remainder. | The negative-`R` consequence is clamped and oracle-gated; the double accrual stands and needs the redemption convention below. The reading gate's docstring names both un-blessed. |
| `pricing` (TARF block) | The target pin fires on 27–61% of paths, 27% short uncorrected, and is gated structurally with no tolerance asserted because nothing resolves it better. Exact behind `Branch_And_Weight: 'Yes'`; the crisp default keeps the declared blindness. | Estimator 13% bandwidth spread, oracle 8.9% flatness — neither better than ~10%. Do not tune on the oracle: it cannot see it either. |
...
| the `Correlations` section vs `save_params` | The two halves of a quanto are authored in different bases and nothing checks they agree: `save_params` emits `ρ̄ᵢ = corr(dW, dWᵢ)`, while the section's rows are the independent normals the cholesky consumes. The section needs `a = ρ̄₁`, `b = (ρ̄₂ − ρ·ρ̄₁)/√(1−ρ²)`; copying `ρ̄₂` in gives a world whose drift and covariance disagree, silently. | `a² + b² = C²` exactly. |
...
| Where | Was wrong → fix, number, gate |
| --- | --- |
...
1. **The TARF observed-fixing redemption convention.** What does a TARF pay on the fixing that
   crosses its target when the crossing is only visible in settled arithmetic later? Blocks half (2)
   of the `use_past_fixing` defect.
2. **The per-fixing smile read.** Sticky-forward moneyness or the deal's declared moneyness — both
...
3. **The compo smile coordinate**, undeclared because every fixture is flat. Same class as (2).
4. **The 36 disagreeing `.field.get` sites** (three fatal): hold a surviving fallback to its
   declaration, or leave the reads as they are. Enumerated in `tests/test_declared_defaults.py`.
5. **Widening the census `FAMILY`** so `pv_MC_Tarf`'s put arm is back in scope now that its flag
   lives in `oss_truncated_draw`. One line.
6. **FVA and CVA in one run.** Turning FVA on scales the set's MTM by survival, which reaches the
   CVA integrand too and moves the shipped `cva` column −0.97% / −2.38% on the gate's sets. One run
   serving both, or the mosaic runs FVA separately.
7. **Component-HN `Quote_Sensitivity`.** Refused: the quote derivative needs IFT through the inner
...
8. **Component-HN positivity.** `q_{t+1} ≥ ω_t + ρq_t − φ − φγ₂²h_t` has no sign for free once
...
9. **Which state an OSS row inherits (F4).** The fix is shallow — the kit's day counter starts at
...
10. **Two rates-emitter design questions**, recorded rather than decided: an OIS block is ~14 MB
...
11. **The α-seed's worst benchmark.** The honesty reprice reads −6.25% against the retired seed's
...
12. **PFE vs CVA measure policy.** CVA is a Q-expectation wanting the market-calibrated outer; PFE
...
13. **`get_implied_correlation`'s two single-caller wrappers.** They would stop two callers building
...
14. **Flagged, not authorised.** `runtime` carries free functions over the hedge bundle in two
    ...
    the same category.
15. **`Boundary_AAD_Window_Touch`'s magnitude.** The switch decides the sign; the magnitude cannot
    be established until `FXPartialTimeBarrierOption` grows `add_grid_dates`.
...
**Conditional-p at a jump, the GBM arm** — form `otm_analytic` from `lognormal_partial_moment` /
`lognormal_fired_gain` on the crisp GBM path with the same supersession at the `boundary_aad` guard.
Under GBM the fixing-interval conditional is exactly Gaussian, so the mixture is verbatim with
`p = Φ(·)`. The type-list gate shape is model-agnostic and already written; the harder HN arm landed
first.
