...
| The RNG-substream ordering invariant has no gate | `config.find_models` → `process_ofs` | [Dependency System](dependency_system.md#rng-ordering) calls this the single most important reproducibility invariant, and reversing the factor order handed to `find_models` leaves the whole suite green. Measured, not surmised: across all 8 CMC test files, 281 of 282 `find_models` calls return 0 or 1 stochastic factor — one factor cannot be permuted — and the two paths that could see it don't (`Hedge_Monte_Carlo` re-derives its own order, `Base_Revaluation` has no stochastic factors at all). A gate needs a fixture with ≥3 stochastic factors of equal topo depth pinning realized draws, not just the order list. |
