...
**Branch and weight — the GBM half is BUILT** (`Branch_And_Weight: 'Yes'` on `BaseValuation`,
default off and byte-identical off, per the supersession ruling; `tests/test_branch_and_weight.py`,
98 gates). What the build found: the OSS loops were ALREADY the smooth estimator for their
knock-out decisions — `oss_truncated_draw` existed with two adopters, the TARF's moving trigger
was already `K + R_{k-1}` — so the landing was the three missing pieces: the TARF's knock-IN
integrated analytically (forced by the one-estimator-per-decision ruling — skipping the
`InnerBoundarySet` without replacing its estimator would have lost the 35.7% of delta the
boundary gates measure), the fired payment by the deal's declared `TargetAdjustment` (a field
nothing had ever read — see the caveat below), and the per-fixing accrual kinks at second order
through the shared kernel. Measured: the two-fixing TARF lands on an independent
Gauss–Legendre region integral at 1.1e-3 / 4.8e-5 / 3.6e-4 / 9.7e-4 relative on value / delta /
gamma / vanna (thirty configurations, three conventions, references built from the DEAL);
variance ratio 11.9× at 4096 paths and 23× at 65536 (the Rao–Blackwell claim, measured);
...
already inert, now gated as byte identity rather than argued). ONE CAVEAT, warned at the read
site and gated two-sided: a TARF declaring `TargetAdjustment` 'Full' or 'None' changes the
DEAL under the switch, not the estimator — the crisp path always paid the capped remainder and
never read the field. THE AUTOCALL IS DEFERRED for a measured reason that replaced the guessed
one: its no-averaging arm is already the construction (one line from the switch), but the put
leg beside the trigger is a bare jump indicator — 18.5–22.3% off its own delta ladder wherever
the barrier is off the strike, exact when on it — and shipping would misname the estimator.
Its unblock is one more leg through `lognormal_fired_gain` (the breach payoff is a lognormal
partial moment); recorded as a two-row gate. HN still refuses by name, citing the stride. The
design record stands below.
