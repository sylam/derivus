...
| The OSS carry strip is not the interval integral | `pricing.sim_spot_oss` (`carry_int = carry * dt`) | `carry[j]` is the AVERAGE annualised rate from `t` to fixing `T_j`; `dt[j]` is the length of the interval **ending** at `T_j`. Their product is that interval's integral only on a FLAT curve. The two sibling adopters of the same seam, `pv_MC_Tarf` and `pv_MC_AutoCallSwap`, both **difference** the cumulative integrals — `(b_j T_j − b_{j−1} T_{j−1})/dt` — and are correct: three adopters, two maintained, one not. It drives the GBM drift of the barrier's own **simulation**, not just its closed-form parity leg, so `E[S_T] ≠ F(t,T)` under the pricer's own Monte Carlo on any sloped curve. Measured **2.85%** on the first row of the sibling fixture; differencing brings the two routes to 2.2e-16. Pinned strict-xfail (`test_payoff_forward_is_wrong_when_the_carry_curve_slopes`), so the fix turns that file red and forces this note to be retired. Hoist the differencing beside `total_log_forward` so all three derive the strip from one expression. |
| Four pricers, and seven of the eight closed-form barrier payoffs, are executed by no test | `pricing` (analytic barrier/option family) | Measured by `gates/pricer_branch_census.py`, ledgered in `tests/test_pricer_branch_ledger.py`: **65** branch arcs across the ten-pricer family that the whole suite never takes. Never run at all: `pv_partial_barrier_option` and `getpartialbarrierpayoff` (no fixture builds an `FXPartialTimeBarrierOption`), `pv_american_option` (every equity fixture is European), and `pv_MC_Tarf.bs_call_put_fwd` (dead — delete it rather than fixture it). `getbarrierpayoff` selects on (direction, eta, phi, strike vs H) and every fixture reaching it is Down-and-Out / Call / K > H, the LAST elif of the OUT block, so all four knock-**IN** formulas and three of the four knock-OUT ones have never been evaluated. **Start with `pv_barrier_option`'s `if direction == BARRIER_IN:` body** — the analytic knock-in leg, zero lines executed, the same construction one function away from the leg that shipped at +1432%, and a fixture for it is one `Barrier_Type` string. |
| The autocall has never been priced at a live carry | `QEDI_CustomAutoCallSwap` fixtures | `gates/fixture_degeneracy.py`: **119 runs** over the barrier/option/HN/TARF/autocall modules, not one at a non-zero rate or a non-zero carry. It is the OTHER adopter of the OSS seam, and its `called`/`knocked` state is the same class of outer path-state override that `hit_value` was — so no fixture that exists could tell a missing `dt` in it from a correct one. One exposure grid at r = 5%, q = 1% closes it. |
| Compo is broken under plain GBM in both OSS pricers | `utils.calc_vol_adjustment`, `pricing.py:1257/2651` | `calc_vol_adjustment` returns the python float `0.0` as its Compo `b_adj`, and both call sites hand it to `torch.unsqueeze` — `TypeError`, deal SKIPPED, so a Compo barrier or TARF has never priced at all. Both also drop its `s_adj` forward scaling. Quanto is unaffected. Pinned strict-xfail in `test_hn_oss_pricers.py`, which turns red the moment Compo starts pricing. |
...
| A zero-length OSS step is not a martingale | `pricing.py:944-945` | `drift` is taken from the **unclamped** variance while `vol = sqrt(var.clamp(min=1e-4))`, and each block's last row has `dt = 0`, so every barrier observation is priced as a σ=1% lognormal kick with no Itô correction: `E[S_j \| S_{j−1}] = S_{j−1}·exp(+5e−5)`. Worth **+0.068%** of the profile here, grid-invariant, and positively identified — an oracle reproducing exactly that floored no-drift step matches the pricer to **+0.000014** where an Itô-corrected floor leaves +0.0024. `drift = carry_int − 0.5·vol·vol` removes the larger half; the complete fix (exact indicator at `times == 0`) consumes a Sobol draw and so moves every barrier constant. Cannot matter for a down-and-out call; **will** matter for a digital or a cash rebate. One half of this IS closed: the parity leg used to reconstruct its carry as `(r + 0.5σ²)`, undoing the folded `−0.5·var` through the floored `sqrt`, which leaked the floor into the **drift** — worth −0.025% of EPE on 13 of 37 rows. Both legs now call `total_log_forward` and never round-trip. The floor's **variance** leak in the step itself is untouched. |
