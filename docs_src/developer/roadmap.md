...
| The HW2F calibration prices every market premium LOGNORMAL and reads neither `Distribution_Type` nor the declared `Shift` | `bootstrappers.create_market_swaps` × `riskfactors.InterestYieldVol` | Found by the rates-emitter build (0de7f12). `create_market_swaps` prices with `utils.black_european_option_price` carrying only `vol_surface.BlackScholesDisplacedShiftValue` — whose property consults ONLY the undeclared `Property_Aliases` and otherwise returns 0.0, never the schema-declared `Shift` — while the DEAL path's `Factor3D.get_subtype` carries `(Distribution_Type, Shift)` into every swaption deal's `Volatility` dependency. So both the distribution AND the displacement diverge between the deal path and the calibration path: a NORMAL-vol ladder (the seeded ZAR `SASN` grid) fits under a lognormal convention, and a block authoring `Shift: 3.0` calibrates at zero displacement, silently. Gated in its current shape by `tests/test_swaption_vol_emitter.py::test_the_engine_prices_a_lognormal_black_and_reads_no_distribution`, so the day it changes the gate says so. Remedy: teach the calibration to read the surface's declared convention — the composition harness's own prerequisite for a live `SASN` fit (the `SASV` lognormal tickers are the interim input, unverified on this workstation). |
...
prints). Its prerequisites are the first two new Known-defects rows above (the premium-convention
row is the hard one: a normal-vol ladder cannot fit honestly under a lognormal calibration), and
its two design questions are recorded rather than decided: an OIS block is ~14 MB live (one
