...
| `num_batches` buys no paths in a risk-neutral IR calibration — it buys wall clock | `bootstrappers.RiskNeutralInterestRateModel.calc_loss_on_ir_curve` | `shared_mem.reset` is called ONCE, before the batch loop, so it clears `t_Buffer` once; `calc_time_grid_curve_rate` keys that memo on curve and time grid rather than on the batch, so every batch after the first re-reads BATCH 0's curve and re-prices the same paths. MEASURED on the identified fixture: prices **bit-identical** at `num_batches` 1, 4 and 8, for N× the wall clock. Found by the Schrager–Pelsser checker's harness, which needed independent batches to compute a standard error at all — so nothing in the tree had ever asked this class for more than one batch. Remedy is one line: clear `t_Buffer` (not `t_PreCalc`, which holds per-solve precalculation) at the TOP of the loop, which is what the harness does. NOT applied: `num_batches` and `batch_size` are undeclared class constants (`= 1`, `= 8192`), so the fix and the fields that would let a job ask for more paths are one decision — and a calibration's path count is a wall-clock ruling, not a patch. The measurement's own reading of what those paths are worth: 0.7–1.1 bp of normal vol per evaluation at 8192 Sobol paths, against 2.2–3.1 bp pseudo-random. |
...
**The HW2F swaption calibration: the α→0 repair is BUILT, and the analytic price exists as a
CHECKER.** `tests/test_hw2f_analytic.py` (78 gates) carries both, and the measurement it was
built to produce is the reason the second is not wired into the objective yet.
...
ρ = −0.95 → ρ\* → +0.95) and is under 0.21 bp at every expiry for tenors ≤ 3Y. **Not wired into
`calc_loss_on_ir_curve`**: on the engine's own device SP is not faster (25 scalar calls lose to
one batched kernel — 0.158 s against 0.140 s on CUDA), so an analytic objective would be bought
for exactness and gradient rather than wall clock, and it wants batching across the benchmark set
first. The full Gauss–Hermite quadrature earns its keep in exactly one place — the
long-expiry/long-tenor corner — and nowhere else.
