...
| `boundary_weights`' near-singularity guard has never refused anything | `pricing.boundary_weights` (`pricing.py:110`) | `usable = denominator.abs() > 1e-30 * (s0*s2).abs()`, but `denominator/(s0·s2) = 1 − s1²/(s0·s2)` is **bounded by 1** by Cauchy–Schwarz, so the threshold sits 25+ orders below anything reachable and the refusal branch is unreachable **by construction**. The docstring states the intent; the code does not implement it. Measured on the HN barrier fixture at 512 paths: one decision's kernel holds **two** points 0.021 widths apart, `rel_den` 8.554e-05, local-linear weights **+50.4 / −49.5** whose jumps do not cancel — that single decision contributes 62.8 of a 55.8 total coefficient and **100%** of the gate's 72.8% gradient error. Guarded, +0.842068 → +1.469847 (0.13% from a 5-rung oracle); ≥1024 paths never refuse and are bit-identical. Not a grid defect: the coarse grid was zeroing that decision (`rel_den` exactly 0, `s0` 9.7e-166) so no gate ever saw it. A candidate guard must test small `rel_den` **and** non-negligible leverage — `rel_den` alone straddles a legitimate 6.2e-03, and ESS alone refuses the wrong decision. |
| The HN barrier's already-hit vanilla is priced with the wrong model | `pricing.py:1238-1251` | `BARRIER_IN`'s `hit_value` prices the knocked-in vanilla with `black_european_option` at the surface vol while the **same pricer's** in-out-parity leg prices that identical state with `hn_call`. Measured 1y ATM, per unit: **10.300022 vs 12.232103 — 15.8% low**. The HN scalars are already in scope. Orthogonal to the scenario grid, which only changes the exposed population (+24.4%). **No AAD-vs-CRN gate can ever see this** — a CRN ladder differentiates the same wrong value — so it needs a value gate against `hn_reference`. |
...
| Two boundary gates pass with their subject deleted | `tests/test_boundary_pricer_events.py`, `tests/test_barrier_bridge.py` | At its pinned point (`'No'`, 512 paths, seed 1) the HN barrier gate passes at **0.31%** with the **entire boundary correction suppressed** — the coarse grid switches the estimator off, so the recorded 0.23% certified nothing. And the discrete-barrier profile gate's statistic has SE **1.40%** at 4096 paths against a **1%** tolerance: it fails on **24/50** seeds under `'No'` and 23/50 under `'Yes'`, so its historical passes were draws. Both need re-baselining under `'Yes'`; the profile gate is better replaced by its own **rebate ledger**, which asserts bit-exactly that knock-out cash lands on the twelve authored days and 0.0000 elsewhere. Separately, the boundary estimator shows **no bandwidth plateau** at 512 or 1024 paths — its documented operating point is 32768 and nobody has run it there. |
...
them, and the flip is what exposed the four defects in the table above, none of which the grid
caused: `'Yes'` is **measurably the more accurate grid**, since interpolating a spot to a barrier
