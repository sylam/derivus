...
**And a third adopter, live today.** `sim_spot_oss` builds its per-interval carry as `carry * dt`; `pv_MC_Tarf` and `pv_MC_AutoCallSwap` — the other two adopters of that same seam — difference the cumulative integrals and are right. Three adopters, two maintained, one not, and this one drives the barrier's simulated drift (see the [roadmap](roadmap.md#known-defects)). One instance is a story; three is the mechanism.
