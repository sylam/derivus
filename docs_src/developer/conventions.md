...
- Where the store's key is not the class name, **the class declares the bridge** rather than the emitter guessing it: a process declares the `factor_types` it drives, a calculation the `calc_type` its JSON `Object` writes (`Base_Revaluation` is authored as `BaseValuation`), a calibration the `model_type` it calibrates, a bootstrapper the `market_factor_type` it selects work by (a block is named `LogVar2FJModelPrices` while its class is `LogVar2FJModelParameters`) and the `price_factor_type` it writes, which keys its `Bootstrapper Configuration` entry (`FXVol`, whose class is `FXVolSurfaceParameters`). No rule recovers any of those names from the other.
...
The one thing wired into that block is [boundary-correction assembly](calc_lifecycle.md#boundary-corrections-the-sensitivity-subsystem), and the terms on which it got there are the terms for anything else: it changes what is handed to `backward()` and **nothing** about what is reported — worth exactly zero in the forward pass by construction, gated bit-identically (`np.array_equal` on the exposure, `==` on the scalar, sensitivities on versus off). A change that cannot make that guarantee does not belong here. The second thing wired in is the [dilution scaling](calc_lifecycle.md#compile-phase-2a-correlation-cholesky-process_ofs) of the correlation matrix, on narrower terms the owner authorised on 2026-09-09: it changes what is reported, but ONLY for a netting set carrying a process that dilutes a correlation and a desk correlation declared on it — a branch taken on `min(D) < 1` with no arithmetic at all on the other side, gated bit-identically at 4,180 floats over 16 documents and on the desk's own xVA pair.
...
| `pv_MC_AutoCallSwap`'s *full-path* branch, behind the other arm of `oss_windows` | its own `carry * dt`, named by no defect report | price unchanged (`r = q = 0` fixture); **rate sensitivities move 100%** |
...
**The moneyness half had a second half.** Reading each PRICER at the moneyness it marks its own Europeans at left the compact autocall reading ONE moneyness for a deal with two strikes in it — a trigger at the initial level and a put at 70% of it — and the put was therefore marked at the ATM vol. On the desk's NKY surface that is 24.6% against 20.3% at five years and **33% of the mark**, measured against the same economics booked as six separate legs, each of which reads its own strike because each is its own deal. The unit is the LEG, not the pricer: `pv_MC_AutoCallSwap` builds a second `forward_vol_strip` at the put's own strike and the put leg walks, survives and pays on it, off the same uniforms, so no other document's stream moves. The barrier OSS pricers already read their European legs at their own strike (`sd_to_expiry = sigma(K, tau) sqrt(tau)`); measured on the skewed NKY surface, an always-in discrete up-and-in matches its own European to **+0.034%** at a 70% strike and +0.022% at the money, so nothing there had to move.
...
**OPEN.** `tests/test_already_hit_barrier_leg_value.py`, the gate the `EquityBarrierOption` JOINT entry was verified against, went in the purge, and the knock-in equity-barrier fixtures that survived it went with the Heston-Nandi retirement. Whether any surviving run still meets the three-way conjunction the entry requires — `single_mtm_row=False` **and** `barrier_dir='In'` **and** `carry_zero=False` — is a question for a run of the plugin, not a grep, and it has not been re-run since the purge.
