...
Usage:
    python experiments/production_walk_forward.py \
        --archive data/pl_exp.csv \
        --calibration-config tests/fixtures/calibration_config.json \
        --marketdata tests/fixtures/data/MarketDataRF_platinum.json \
...
# Must precede the first torch/derivus import (pulled in via production_solver below):
...
# validated objective (matches tests/fixtures/platinum_hedge_shipping.json)
...
def garchify_md(hmm_md, arch, cal_end, out_md):
    ...
    from derivus.stochasticprocess import GARCHSpotCalibration   # the framework's calibrator
...
def main():
    ...
    ap.add_argument('--calibration-config', default='tests/fixtures/calibration_config.json', help='Calibration config JSON.')
    ap.add_argument('--marketdata', default='tests/fixtures/data/MarketDataRF_platinum.json', help='Source MarketDataRF (Model Configuration).')
