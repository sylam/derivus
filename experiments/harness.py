...
Useful breakpoints:
  derivus/hedge_solver.py   DiffSolverV2.solve        (driver: bank -> forks -> fit -> verdict)
  derivus/hedge_solver.py   DiffSolverV2._fit_step    (per-t bootstrap + twin-loss fit)
  derivus/hedge_solver.py   DiffSolverV2._decide      (argmax; cost-aware branch)
  derivus/hedge_solver.py   DiffSolverV2._verdict     (OOS rollout vs textbook/no-hedge)
  derivus/calculation.py    _run_inner_mc_at_t        (the inner-MC fork; one-step window)
  derivus/pricing.py        pv_energy_cashflows       (ForwardCurve=Components liability)
"""
import json
import logging
import os
