...
Usage:
    ...
    python experiments/production_solver.py --config policy.json --save value_fn.pt

    # train an ensemble (one checkpoint per seed -> value_fn_s7.pt, value_fn_s42.pt, ...):
    python experiments/production_solver.py --config policy.json --save value_fn.pt --seeds 7 42 314

    # scale the batch up on a bigger GPU (the recipe is stable to at least 8192; 16k needs care):
    python experiments/production_solver.py --config policy.json --save value_fn.pt --batch 16384

    # evaluate a frozen ensemble on fresh paths (no training):
    python experiments/production_solver.py --config policy.json --load value_fn_s7.pt value_fn_s42.pt
...
describes. The shipping deliverable (tests/fixtures/platinum_hedge_shipping.json) is the corrected
...
# imported (derivus pulls torch below) — a later setdefault would be too late to take effect.
