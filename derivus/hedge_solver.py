...
class HedgeActionSpace:
    """The solver-owned ACTION UNIVERSE and friction access, built ONCE from `(runtime, device)`
    and shared by every track — DiffSolver, HindsightDpSolver, `run_textbook_benchmark` and the
    stepper rollout — so they optimize over the same positions and price the same frictions.

      * `axis_levels()` / `grid()` — the MASK-AWARE target-position grid: active hedge axes span
        `[min, max]` at `levels` points, inactive axes pin to a single 0, and rows over the
        total-position cap are dropped.
      * `allocation_grid(t)` — the FACTORED alternative under `Allocation_Weights`: one ladder over
        ...
        on which universe is configured; downstream consumes the rows either way.
      * `kappa(tradables_sim, t)` — the per-hedge turnover kappa at sim-grid `t`, off the single
        `hedge_runtime.per_contract_kappa` rule.
      * `initial_q(batch, device)` — the opening book `q0`.
      * `turnover_cost(dq, kappa)` / `schedule_key(schedule)` — the friction charge on a reposition
        and the comparison-stable corridor stamp."""
    ...
    def __init__(self, runtime, device, vol_sim=None):
        ...
        self.device = device
        # sim-grid per-step annualized vol series (or None), threaded through `kappa` into
        # `per_contract_kappa`'s Vol_Scale
        ...
        # optional per-leg rate limit on |dq| per decision step (0 = off). EXECUTION policy only:
        # the label path passes rate_limit=False, so the training argmax never arms the band.
    ...
    def _weights_at(self, t, live=None):
        ...
        The live intersection happens HERE, before the ladder is built, rather than as a filter
        afterwards: a knot's weight on a dead leg is cover the book cannot realize, so leaving it
        in makes every rung advertise a net it masks away from and makes a corridor the live legs
        can satisfy look infeasible."""
        w = self.weights[0][1]
    ...
    def _largest_remainder(x, target):
        ...
        for a row that must be `-10`. Flooring is toward −∞ so the leftover is non-negative and a
        short book is apportioned by the same expression as a long one; the leftover is also
        strictly smaller than the number of legs carrying a remainder, so a leg already on an
        integer box edge never receives a unit and no row leaves its box."""
        base = torch.floor(x)
    ...
    def allocation_grid(self, t, live=None):
        ...
        The Cartesian grid's quantum on the NET is the per-axis step times the number of legs, so a
        3-leg 9-level book moves the net in jumps larger than any realistic friction can smooth.
        `Training_Action_Grid_Levels_Per_Axis` counts rungs on the TOTAL here instead.

        Construction runs on the legs that can carry a contract — `_weights_at(t, live)` drops the
        inactive and expired ones and renormalizes, and every bound is summed over what survives:
          * the band is `net_bounds(t)` (cap ∩ corridor ∩ boxes) intersected with those legs' own
            boxes, then snapped INWARD to whole contracts, since a fractional band would emit
            rungs that round outside their own mandate;
          * `Q·w` is clamped into the per-leg boxes and water-filled back onto Q over the SAME leg
            set, so nothing puts cover on a leg the schedule zeroed;
          * `_largest_remainder` rounds to whole contracts summing to Q, and duplicate rungs
            collapse.
        Cached per `(t, live)`. The corridor filter downstream in `grid_at` is then a no-op: the
        rungs a caller is offered are rungs it can realize."""
    ...
    def grid_at(self, t, live=None):
        ...
        in the `Total_Position_Schedule` corridor at t. No schedule → that grid unchanged. The
        single per-t filter site every track shares; empty after filtering ⇒ infeasible corridor.

        The corridor bounds the REALIZED signed total, the position that survives expiry masking,
        so passing the step-t `live` leg mask filters on `Σ(q_i·live_i)`: a corridor-satisfying
        short cannot be parked on an expired leg only to be masked to 0 and silently under-hedge.
        Absent `live` ⇒ `Σq_i` over all legs."""
    ...
    def project_to_corridor(self, q, t):
        ...
        its box. Feasible whenever the corridor is grid-feasible. No schedule → `q` unchanged.

        Where `grid_at` FILTERS a discrete action universe for the argmax, this CONTINUOUSLY nudges
        an exploration or benchmark position: the bank rolls continuous q around the min-var hedge,
        so a filter has nothing to select from."""
        if self.schedule is None:
            return q
    ...
    def static_grid(self):
        ...
        entry-step grid. Under `Allocation_Weights`: every knot's ladder, deduped and held to the
        ENTRY corridor — every composition the schedule offers, any one of which a hold could have
        carried from day one.

        Not just the entry knot: the DP recomposes at every knot and a constant hold cannot, so
        pinning the benchmark to the entry ray would put an action-space difference inside the
        DP-vs-textbook gap. The hold still cannot TRACK the schedule, only choose which
        composition to sit in."""
    ...
    def turnover_cost(self, delta_contracts, kappa, kappa_cal=None):
        ...
        Hard, with no smoothing: the action search has no gradients on the action.

        This is what lets a policy roll: L1 charges a roll as two outright crossings, so a book
        that should move down the curve is held into expiry instead. `hedge_bundle._roll_rebate`
        calls the same `turnover_charge`, or the policy would be optimized against a friction the
        book never pays."""
...
def run_textbook_benchmark(bundle, runtime):
    """Static-hedge reference: the single best CONSTANT position, held over the whole horizon with
    no rebalancing, evaluated FRICTIONLESS on the realized outer paths — a valid lower bound for
    the dynamic DP, since rebalancing can only add value. No inner MC and no V̂; a static hold
    telescopes the per-step P&L to `position · (F_T − F_0)`. Shares `HedgeActionSpace`, so it
    respects `Active_Hedge_Indices` exactly like the greedy grid.

    The turnover a real execution would pay — entry from the OPENING book `q0` plus a terminal
    unwind at the PRE-SETTLEMENT row, the one row all three tracks unwind at so their `*_net`
    columns compare — is a net-of-cost DIAGNOSTIC, never charged against the V_0 track.

    The corridor filter is the ENTRY-step one: the hold is chosen once at entry, and a per-t
    corridor is a dynamic constraint a constant hold cannot track. Under `Allocation_Weights` the
    universe is `aspace.static_grid()`, every knot's composition rather than the entry knot's, so
    an action-space difference does not land inside the DP-vs-textbook gap."""
    ...
    # entry-step corridor only: a constant hold cannot track a per-t one
    ...
    if acc["force_flat_at_end"]:
        # unwound at the PRE-SETTLEMENT row, since the last row is the post-settlement clean exit
        # where there is nothing left to liquidate. Outright: flat against the market, not leg
        # against leg.
...
class HindsightDpSolver:
    ...
    `u_signed(W_T)` is maximizing the additive cash `G_T`; with no turnover cost the per-step
    choices decouple and the max-plus DP collapses to a per-step argmax. `mean_b V_0(b)` is an
    upper bound on any deployable policy's value. The turnover a real execution of this bang-bang
    trajectory would pay — entry from the OPENING book, per-step repositioning, terminal unwind at
    the PRE-SETTLEMENT row — is a net-of-cost DIAGNOSTIC, never charged against the V_0 track.
    Shares `HedgeActionSpace`, so it respects `Active_Hedge_Indices` like the greedy grid."""
    ...
    def solve(self):
        ...
        # per-t action universe. The net-of-cost trajectory is accumulated for the diagnostic.
...
class _DiffV2Residual(torch.nn.Module):
    """Zero-init residual head `A_t(market, q, W)`. The continuation is `C = u(W) + A`, so a zero
    final layer makes C start exactly at the bounded utility anchor. Under the successor chain only
    the TERMINAL net starts there; every earlier net starts at its fitted successor, and the
    trust-region clamp (`a_bounds`) is the brake covering the inherited starts."""
...
class DiffSolver:
    """Differential-ML hedging solver: a backward DP whose per-step value is fitted by the
    Huge–Savine twin loss. All dynamics come from the bundle's inner-MC closures
    (`Bundle.inner_mc` / `Bundle.inner_mc_grad`), which fork the simulator and price through the
    ordinary deal pricers — no analytic transition and no Jacobian reconstruction.

    Wealth convention: `W_t` = cumulative hedge P&L + the marked liability `L_t`, stepping as
    `W_{t+1} = W_t + Σ_i q_i·cs_i·(F_{t+1,i} − F_{t,i}) + (L_{t+1} − L_t)`, with terminal utility
    `u(W_{T_dec})`.

    The design:
      * `C_t(market, q, W) = u(W) + A_t(market, q, W)`. `u` is the bounded utility anchor and `A`
        a zero-init residual net — the only learned part. The bounded anchor is what keeps the
        backward recursion from running away.
      * External argmax — the Bellman max is a discrete grid search over target positions OUTSIDE
        the fitted value, so the net never has to represent an argmax.
      * Advantage decomposition — fit `A = C − u(W)` on the value AND the wealth-channel pathwise
        gradient, so the unbounded residual cannot drift off u.
      * Operating-region bank — roll the OUTER paths forward exploring q around the per-t
        replication hedge, so wealth stays in-band and A stays on-support.
      * Position-free value (the default) — with no turnover cost the held position is a freely
        reset control, entering the value only through next-step wealth. The hedge instruments
        live in the ACTION grid and the wealth step, not as a state coordinate.
      * FRICTIONAL Bellman (`DiffV2_Position_State`) — once turnover costs money the incoming
        position IS a state variable, so the net gains the signed net book fraction
        `p = Σq/Q_max` and the repositioning charge is subtracted from the wealth that becomes
        the regressed TARGET. A charge levied only at the argmax is a one-day toll the value
        function never remembers, and no no-trade region can form out of a toll.
      * RUNNING WEALTH (`Objective.Reference_Mode`) — the reward becomes the DAY's wealth
        increment, `Y = u(W1 − W_t) + E[C_{t+1}]`, with `C_{T_dec} ≡ 0` and `C_t = A_t` alone.
        Terminal wealth drifts outside the utility's knee by mid-horizon, which leaves the
        objective affine and the argmax bang-bang; an increment is O(one day's move) every day.
      * CONDITIONAL-SIM SCALE (`Objective.Utility_Scale_Mode`) — the same defect attacked from the
        scale: a per-decision-step knee `c_t` measured off the warmup batch, so `x = (W − R)/c_t`
        is a z-score on every day. It owns the LEVEL of c as well as its shape, which RE-DOSES the
        dimensionless aversion. Independent of the mode above and REFUSED beside it.
      * WEALTH-FREE residual (`DiffV2_Wealth_Free_Value`) — `A_t` loses its W input column, so the
        ranking's whole wealth dependence is `u`'s own. A residual free to carry a wealth slope
        over the action-induced span adds a term the anchor's curvature has to fight.
    """
    ...
    def __init__(self, bundle, runtime):
        ...
        grid appends one post-settlement clean-exit row, so the meaningful terminal is the
        bundle's `last_live_mtm_index`. Telescoping wealth THROUGH the settlement drop would
        cancel the liability's settlement risk. Decisions run 0..T_dec-1; the terminal
        continuation marks at T_dec."""
        self.bundle = bundle
        self.runtime = runtime
        self.cfg = runtime["solver"]
        self.device = bundle.device
        # the SAME action universe the benchmark tracks and the stepper rollout consume, so every
        # track optimizes over identical positions and prices identical frictions
        ...
        # downside-aware action selection; 0 = plain E[C] argmax
        self.risk_kappa = float(self.cfg.get("diffv2_risk_kappa", 0.0))
        # cost-aware EXECUTION: the verdict rollout charges kappa*|q - q_prev| at the argmax;
        # training and selection stay cost-free
        ...
        # FRICTIONAL BELLMAN: the net book fraction p = Sum(q)/Q_max becomes a state coordinate and
        # the repositioning charge enters the regressed target. Q_max is the Evaluator's cap - the
        # only declared scale p can be measured in - so an absent cap fails loud.
        self.position_state = bool(self.cfg.get("diffv2_position_state", False))
        # WEALTH-FREE residual: the value net's W input column is REMOVED, so C_t's whole
        # dependence on wealth is the utility anchor's. Composes with the position state.
        ...
        # the two OBJECTIVE dials, read off the normalized runtime. RUNNING WEALTH rewards the
        # day's wealth INCREMENT, which makes C_t a sum of per-step rewards and retires the u(W)
        # anchor; SCHEDULED SCALE gives each decision step its own knee.
        ...
        # scale-free, its domain the wealth floor the forward-constructed bank maintains
        self.log_ratio = obj.get("object") == "logwealth"
        self.w_floor = 1.0
        # the DP's aversion: it divides the capital line in the LogWealth reward, so gamma > 1 puts
        # less capital at risk. 1.0 is neutral, and it is inert under the other objectives.
        ...
        # early termination for non-anchor fits: a BLOCK-wise relative loss plateau (blocks of 8
        # iters, floor of 2), so a cold-Adam stall cannot read as convergence. The terminal anchor
        # always runs the full budget; 0 disables.
        ...
        # per-t Adam optimizers, created at the first fit of t and KEPT, so a streaming step
        # continues the same moments on the same net
        self._opts = {}
        # the locked frame. `utility_scale` is None until the first bind completes, which is what
        # marks that bind as the warmup one; `_bounds_frozen` turns `a_bounds` from
        # fitted-per-batch into frozen-and-reported.
        self.utility_scale = None
        self._bounds_frozen = False
        self._breaches = []
        self._bind(bundle)
        # effective terminal = the last LIVE liability mark, not the last row
        ...
        # re-binds to a batch that measured its own, which must not reach the reward.
    ...
    def _bind(self, bundle):
        ...
        LOCKED utility scale and its per-step schedule are re-asserted — every batch resolves its
        own frame at build time, and letting a later one reach the runtime would rescale the
        objective, and every fitted C_t with it, mid-training."""
        self.bundle = bundle
        ...
        if self.log_ratio:
            # a hair above zero on the book's own dollar scale: a ratio against a launch below
            # this is ruin, and the reward there is capped at zero
    ...
    def _member_anchor(self, W, t, schedule):
        ...
        Under a step-sum objective (Running_Wealth / LogWealth) the continuation carries NO
        terminal utility, so the member anchor is exactly zero rather than a wrap call the growth
        objective refuses by name."""
    ...
    def grid_size(self):
        ...
        `diagnostics['action_grid_size']`. A property rather than a construction-time int: under
        `Allocation_Weights` the Cartesian meshgrid must never be built at all (51 levels on 5 legs
        is ~345M rows for a 51-row ladder), and its size would be the wrong number anyway."""
    ...
    def _standardize(self, market, W, p):
        ...
        `aspace.net_bounds(t)` — so p arrives in [-1, 1] over a support the bank covers.

        `DiffV2_Wealth_Free_Value` drops the W column instead of adding one. The two switches
        compose, each owning one column, so the layout is `(market | [W] | [p])` in that order
    ...
    def _continuation(self, nets, market, W, t, p, chunk=400_000):
        ...
        Under `DiffV2_Wealth_Free_Value` the residual is `A_t(market[, p])` and W enters through the
        anchor alone; `_standardize` owns the layout either way.

        Under `Objective.Reference_Mode='Running_Wealth'` the anchor is IDENTICALLY ZERO and
        `C_t = A_t` alone, with terminal `C_{T_dec} ≡ 0`: once the reward is the day's increment
        there is no terminal utility of the level to anchor on. The per-step reward is added by the
        CALLER, the only place holding both W1 and the wealth it moved from.

        Ensemble mode (list-of-checkpoints load): `A` = mean over members, each evaluated in its
        OWN standardization frame — which under a `conditional_sim` schedule includes that member's
        own step knee.

        `A_t` is CLAMPED to its fitted-target trust region (one range-width of headroom): off
        support the zero-init MLP extrapolates freely, and the argmax then chases a phantom
        direction that poisons the t−1 bootstrap labels. Outside the clamp the gradient is zero,
        so the differential labels ignore those directions too."""
        ...
        if ensemble:
            ...
            for m_nets, m_mean, m_std, w_mean, w_std, m_bounds, m_sched in ensemble:
                if m_sched is not None:
                    # the member's own step KNEE, as much a part of its frame as its z-stats.
                    # Written as a correction to the shared anchor so a schedule-free ensemble
                    # stays arithmetically untouched.
                    acc = acc + (self._member_anchor(W, t, m_sched) - base)
                if t >= self.T_dec:
                    # the TERMINAL has no residual, so this correction is the WHOLE of the
                    # member's continuation - which is what the last decision is ranked on
                    continue
    ...
    def _reposition_charge(self, acts, q_prev, kappa, kappa_T, kappa_cal=None):
        ...
        ONE expression, called from every seam: the wealth that RANKS a candidate in `_decide`, the
        wealth the INCUMBENT holds against it, and the wealth that becomes the chosen action's
        regressed TARGET in `_fit_step` all price the same friction. That equality is the whole of
        the frictional Bellman. Callers pass LIVE-MASKED positions on both sides, so a dead leg has
        `dq = 0` and carries no phantom toll.

        `kappa_cal` (from `_calendar_kappa`) makes the toll SPREAD-AWARE: the move's matched half
        pays the calendar rate rather than two outright crossings. The terminal unwind stays
        outright, taking the whole book to flat against the market rather than leg against leg."""
        dq = acts - q_prev
    ...
    def _u_step(self, W1, W0, t):
        ...
        # the CAPITAL line: wealth in the ratio is capital + MTM, so "wealth 1" is one unit of
        # backing capital and ruin is losing it. Both legs sit on the SAME step's c, so the inner
        # fork is measured against its own launch.
        ...
        # BOUNDED CONDITIONING: the log's economic content lives within a few nats, and beyond
        # ruin there are only magnitudes that wreck the fit. -20 nats is ruin by e^20, so the
        # cliff stays a cliff and the labels stay a distribution.
        u = u.clamp_min(-20.0)
        return torch.where((c + W0) > self.w_floor, u, u.clamp_max(0.0))


    def _set_displacement(self, W_bank):
        """The log's bias, MEASURED FROM THE FORWARD PASS: the worst book any simulated bank path
        touches, plus a 10% buffer. Wealth stays free to go negative, the shifted log is defined on
        everything training will see, and the -20-nat backstop guards only the rare inner-fork dip
        beyond the buffer.

        THE HEADROOM GATE: the capital line is `c = displacement/aversion`, so the worst path sits
        at headroom `(c + worst)/c = 1 - aversion/1.1`. Aversion rescales the very buffer the
        displacement provides — at 1.1 the buffer is gone and every worse outcome lands on the
        saturated tail — so a domain break refuses and thin headroom warns."""
    ...
    def _capital(self, t):
        """The DISPLACEMENT of the growth reward — a shifted-lognormal bias term: one constant per
        run, the scalar utility scale, which is by contract the TERMINAL dispersion read.
        Deliberately t-independent, because a per-step capital line would change the objective's
        curvature day by day; the shift only keeps the log's argument positive and the curvature
        stable. `t` is accepted for the call sites' sake and ignored."""
    ...
    def _wealth_step(self, W, q, dF, dL):
        ...
        `hedge_bundle.wealth_step`, which `_futures_account_step` discretizes. `q` and `dF` are
        `(..., n_hedge)`, `dL` is `(...)`. Expiry is composed by callers through `live` on dF."""
    ...
    def _inner_step(self, t):
        ...
        EXPIRED-CONTRACT GUARD (the returned `live` mask): the framework returns inner `F_t1 = 0`
        for a tradable that expired before the fork's t+1, while the OUTER `tradables_sim` FREEZES
        at the last traded price, so a naive `F_t1 − F_t` mints a spurious ~−F_t move a short would
        mine fake P&L from. A dead contract's one-step move is forced to 0; `live_i` says the
        contract still prices at t+1."""
        ...
        # EXPIRED-CONTRACT GUARD: a dead contract's one-step move is forced to 0
    ...
    def _decide(self, nets, market_t1, dF, dL, W, t, q_prev=None, kappa=None, live=None,
        ...
        instead — the same expression the LABEL regresses on in `_fit_step`, so the ranking and the
        labels cannot score different things.

        The action universe is `aspace.grid_at(t, live)`: the base grid filtered to the
        `Total_Position_Schedule` corridor at t, or the FACTORED Q-ladder under
        `Allocation_Weights`. The `live` leg mask enters the filter so the corridor bounds the
        REALIZED `Σ(q_i·live_i)` rather than a target the expiry mask then guts.

        `q_prev` carries the standing book and arms up to four decision-time corrections, each with
        its own switch: the cost-aware turnover charge (`kappa` present, priced net/paired when
        `Calendar_Spread_Bps` gives `_calendar_kappa` a rate), the quadratic churn charge, the
        `Max_Trade_Per_Step` band (`rate_limit` True, so the label path keeps the band
        execution-only) and the `Decision_Deadband_Sigma` incumbent. The band takes the best action
        INSIDE the reachable set and falls back to the unrestricted best when nothing is reachable.
        Under `Allocation_Weights` it measures `|ΔΣq|` rather than per-leg `|Δq_i|`, because a
        composition jump at a knot is the schedule's mandate rather than a trade the argmax chose.
        Without `DiffV2_Position_State` these shape SELECTION only and the fitted targets stay
        cost-free; with it, the same charge plus the terminal unwind enters the label's target and
        the successor is queried at `p' = Sum(a·live)/Q_max`, the book each candidate leaves.

        EXPIRY, one convention end to end: every position this method prices, states and RETURNS is
        LIVE-MASKED — candidate, standing book, incumbent, band and answer alike. A dead leg cannot
        be traded and earns nothing, so an unmasked `q_prev` would let the argmax park size on a
        dead leg purely to dodge the phantom's unwind, spending the cap on nothing.

        `Decision_Deadband_Sigma` makes the standing book the INCUMBENT candidate: the winner only
        trades when it beats holding by that many standard errors of the difference PAIRED across
        the common inner draws. Holding an infeasible book is not on offer — a path whose `q_prev`
        sits outside the corridor at t moves regardless — and the incumbent runs through the SAME
        `_reposition_charge` as every rival, so the paired difference subtracts two readings of one
        function. The move test compares RISK-NEUTRAL means even when `DiffV2_Risk_Kappa` is on: a
        paired standard error is a statement about E[C], and a semideviation is not a mean."""
        ...
        with torch.no_grad():
            ...
            # one expiry convention: the standing book, like every candidate, is only what lives
            ...
            for s in range(0, grid.shape[0], self.chunk):
                ...
                # the successor is queried at the book the CANDIDATE leaves standing
                ...
                if self.running_wealth:
                    # the DAY's reward, read off the CHARGED W1 so a move pays its toll inside the
                ...
                if limit:
                    ...
                    # under a weight schedule the control is the NET cover, so the band measures
                    # |dSum q|: a per-leg band would strike out every row at a knot step
            ...
            if dead:
                # the incumbent is a CANDIDATE priced by the same rules: dq = 0 kills the L1 and
                # churn terms, and only the terminal unwind - a charge on the LEVEL - survives
                ...
                C1f_inc = self._continuation(
                    ...
                    p_inc).reshape(B, Bi)
                if self.running_wealth:
                    # the incumbent earns its own day's increment, priced as every rival's
    ...
    def _replication_hedge(self, t):
        ...
        for i in self.active:
            ...
            var = dF.var()
            # skip degenerate OR non-finite instruments: an expired-contract row can carry a NaN
            # mark, and clamp(NaN) = NaN would poison the whole textbook book
    ...
    def _build_bank(self, gen):
        ...
        Under `DiffV2_Position_State` the bank also has to COVER THE p AXIS: p is an input the
        argmax queries over the whole of `aspace.net_bounds(t)` while the roll only wanders
        `Bank_Noise_Frac` around the replication hedge, which would leave the learned hysteresis
        slope extrapolating where it matters. Each path therefore draws a UNIFORM net target across
        the feasible band and water-fills onto it, so the per-leg shape stays the noisy replication
        hedge, the total spans what the argmax can ask for, and p_bank is inside [-1, 1]."""
        L = self.liability_sim
        W = L[0].clone()                                                                 # (B_outer,) = cum_pnl(0)+L_0
        # seed q_prev from the OPENING book. Position-free this only labels q_list[0]; under
        # `DiffV2_Position_State` q_list[t] IS the sampled position state at t.
        ...
        for t in range(self.T_dec):
            ...
            if self.position_state:
                # cover the p axis the argmax queries. Subsumes the corridor projection below,
                # since net_bounds already intersects the corridor.
            ...
            if self.aspace.schedule is not None:
                ...
                # states only; the diagnostic below confirms ~0 residual breach
    ...
    def _project_leaf_grads(self, leaf_grads, widths, rows, n, md):
        ...
        the privileged market_t columns the value net consumes. `widths` is in market-column order
        as `(key, width, state_suffixes)`, the suffixes being the process's DIFFERENTIABLE state
        coordinates. Layout: state columns first in declared order, the trailing price columns
        last. Factors with no state suffix map 1:1 raw to privileged or leave the non-price state
        masked; unmeasured or unconnected leaves leave their columns at 0, supervised by the value
        alone."""
    ...
    def _fit_step(self, nets, W_bank, t, inner, q_bank, rows=slice(None), live_prev=None):
        ...
        The GRAD inner-MC fork returns AAD-live one-step `F_t1`/`L_t1`/`market_t1` plus the
        per-process state-at-t LEAVES, so the bootstrap value Y and its pathwise gradients w.r.t.
        W0 and the market state come from the SAME forward — the full Huge–Savine twin loss.
        `∂Y/∂market_t` is the differential constraint regularizing the market dimension, where a
        value-only fit overfits the few outer paths.
        ...
        exactly `u(ΔW_T)` — the same expression `_decide` ranks candidates by.

        Under `DiffV2_Position_State` the step is FRICTIONAL: the bank's own standing book supplies
        the position state p at t, the selection is charged its repositioning cost, and the SAME
        charge is subtracted from the wealth entering the target, whose successor is read at the
        book the chosen action leaves standing. The toll then compounds down the recursion instead
        of being re-paid and forgotten each day, which is what distinguishes this from
        ...
        — the book masked by the step that SET it, not by the step about to trade it. `_decide` at
        t−1 can only hold its own `live`, so it hands the successor `Sum(a·live_{t-1})/Q_max` and
        the fit has to meet it there; masking with this step's `live` would read `nets[t]` on one
        coordinate and write it on another, disagreeing by the full width of the axis on exactly
        the step where a leg rolls off. `live_prev` carries that mask in. The CHARGE keeps this
        step's `live`: what you own and what you may trade are different questions."""
        dF_ng, dL_ng, m1_ng, market0, live = inner                                       # no-grad cache
        market0 = market0[rows]
        W0_bank = W_bank[t][rows]
        # live-masked like every other caller: an expired leg's phantom book must not buy position
        # budget. With churn_lambda 0 and no kappa the book is inert.
        q_prev = q_bank[t][rows] * live
        # ...but the position STATE is the book masked by the step that set it
        ...
        # SELECT the action on the NO-GRAD inner draws; EVALUATE its value and pathwise gradients
        # on a fresh GRAD inner, so independent draws cross-fit away the winner's-curse max bias
        ...
        # GRAD inner-MC fork: AAD-live t -> t+1 quantities plus the state-at-t leaves
        ...
        if self.position_state:
            ...
            # successor read at the book the chosen action leaves standing
            W1 = W1 - self._reposition_charge(
        ...
        if self.running_wealth:
            # Y = u(W1 - W_t) + C_{t+1}, the same expression `_decide` ranked with. The increment
            # is level-free, so it contributes 0 to the wealth label and its MARKET slope joins
            # g_market - the reward is part of what the twin loss differentiates.
    ...
    def _fit_from_labels(self, nets, W0_bank, market0, Y, gW, g_market, t, q_star, p_bank):
        ...
        matching is mis-scaled — `∂A/∂W` ~ 1e-6 in dollars and `∂A/∂spot` ~ 1e-4, both inert
        against the O(0.1) value term — while standardized both are O(1) and the gradient match
        actually regularizes.

        Each term is normalized by its own label variance: with W ~ $1e6 and utility ~ O(1) the
        standardized W-gradient label is ~600x the value label, so an unnormalized sum would let it
        drown both the value fit and the market gradient, and `lam_g` would not balance
        value against gradient as intended.

        `p_bank` is an input column with NO gradient label: the twin loss carries only the channels
        the AAD fork measures, and p is a control the bank samples rather than a simulated
        coordinate. It is supervised by the value term alone.

        `DiffV2_Wealth_Free_Value` is the same discipline the other way: W is no longer an input,
        so it can no longer be a MEASURED COLUMN and its gradient term leaves the loss with it.
        Matching `∂A/∂wn` against a measured `g_zn_W` while wn is absent would ask the market
        columns to absorb the wealth slope, which is the slope the switch exists to remove."""
        # advantage decomposition: fit A = C - u(W0), subtracting the anchor's wealth slope. Under
        # `Reference_Mode='Running_Wealth'` C_t = A_t alone, so there is no anchor to subtract.
        ...
        net = nets[t]
        # the chain: the terminal net anchors and trains the full budget, while every earlier net
        ...
        # twin loss in STANDARDIZED space: g_zn = std*g_raw
        lam_g = float(self.cfg.get("diffv2_lambda_grad", 1.0))
        g_zn_W = self.w_std * a_gW                                                       # (B,)
        g_zn_m = self.m_std * g_market                                                   # (B,md); u indep of market
        # per-term label-variance normalization
        nrm_v = a_val.var() + 1e-8
        nrm_w = g_zn_W.var() + 1e-8
        # per-column lambda_j: each market column normalized by its own label variance, so one
        # fat-tailed column cannot deflate the differential constraint for the rest
        nrm_m = (g_zn_m.var(dim=0, keepdim=True) + 1e-8
                 if bool(self.cfg.get("diffv2_per_column_grad_norm", False))
                 else g_zn_m.var() + 1e-8)
        # one optimizer per t, so a streaming step resumes the same Adam moments on a warm-started
        # net instead of restarting them on every batch
        ...
        for it in range(self.fit_iters):
            ...
            # consumes, so `DiffV2_Wealth_Free_Value` takes W out of both at once
            ...
            # block-wise early stop, non-anchor only: one host sync per 8-iter block, a 2-block
            # floor so a cold optimizer cannot plateau its way out, and a stall test on block ends
        ...
        with torch.no_grad():
            ...
            # trust region for every future EVALUATION of this net: its fitted-target range plus
            # one range-width of headroom each side
            lo, hi = float(a_val.min()), float(a_val.max())
            if self._bounds_frozen:
                # streaming: the region is part of the LOCKED frame, since re-fitting it per batch
                # would re-open the phantom-extrapolation basin the clamp exists to close
    ...
    def _verdict(self, nets, inner_cache, sweep_ts, rows=slice(None)):
        ...
        Returns per-policy `{u_mean, wT_mean, wT_p5, wT_cvar5}`. The greedy policy should DOMINATE
        no-hedge on downside and stay competitive with textbook; a SPECULATING policy shows up as
        worse p5/CVaR than textbook with a wide wT spread.

        Under `Objective.Reference_Mode='Running_Wealth'` `u_mean` is the SUM of the rollout's
        per-step utilities, because that is the objective the DP maximised. It is therefore NOT
        comparable to a terminal-utility `u_mean`, nor to `run_textbook_benchmark` /
        `HindsightDpSolver`, which stay terminal-based. `wT_mean` and the tail columns are raw
        dollars under either mode; `u_mean_is_step_sum` says which regime produced the number.

        Turnover cost is accounted in PARALLEL — entry from the OPENING book `q0`, per-step
        repositioning, and the terminal unwind under `Force_Flat_At_End` — the same three-part rule
        the two benchmark tracks price their execution with, so all three `*_net` columns compare.
        The wealth recursion itself stays cost-free, so these are DIAGNOSTICS on that
        approximation.

        The greedy ARGMAX is charged when either `DiffV2_Cost_Aware_Argmax` or
        `DiffV2_Position_State` is on: a value function fitted against a charged selection must be
        rolled under the same rule, or the policy reported is not the policy trained.
        `argmax_charged` says which regime produced these numbers."""
        L = self.liability_sim
        t0 = self.t_min
        n = L[t0][rows].shape[0]
        W = {p: L[t0][rows].clone() for p in ("greedy", "textbook", "nohedge")}
        q_traj = {"greedy": [], "textbook": []}                                          # mean |q| per step
        # parallel turnover-cost DIAGNOSTIC, entry measured from the opening book
        ...
        # running-wealth: the rollout's value is the SUM of the per-step rewards, accumulated along
        # the roll, and the net sum charges each step's own turnover into that step's increment
        ...
        with torch.no_grad():
            for t in sweep_ts:
                ...
                # in-sim validation CUSUM: the fork's expectation minus the outer realized move,
                # averaged over paths and summed over steps - a mean-zero tripwire within the
                # model, whose deployment twin is the stepper roll's market-drift monitor
                ...
                # textbook = diagonal min-var, PROJECTED into the corridor so the benchmark obeys
                # the same mandate as greedy (no schedule = identity)
                ...
                if u_run is not None:
                    # indexed by the step the increment ARRIVES at, as every other seam does: the
                    # knee an increment is measured under belongs to the day it lands on
                    for p in W:
            ...
            if self.force_flat:
                # the unwind both benchmark tracks charge, so all three price one execution
                ...
                for p in cost:
                    ...
                    if u_net is not None:
                        # the liquidation belongs INSIDE the last increment, where training charged
                        # it; u is nonlinear, so appending u(-unwind) as its own term would report
                        # a different objective than the one optimised
                        u_net[p] = (u_net[p]
        ...
        # THE NULL: the per-step std of the within-model standardized residual, so the tripwire
        # boundary at inference is k * null_sigma * sqrt(t). Stamped with the frame.
        if self.loaded is None:
            # measured on TRAINING validation only, where the fork and the outer paths share one
            # law. On a LOADED roll the same statistic reads model-vs-market drift, and stamping
            # that as the null would inflate the tripwire's threshold with the signal it detects.
        ...
        # which regime chose the greedy book, and whether `u_mean` is a terminal utility or a sum
        # of per-step ones - neither is readable off the figures
    ...
    def _score_actions(self, nets, t, acts, market_t1, dF, dL, W, q_live, kappa, live):
        ...
        A SECOND PASS rather than a probe wired into `_decide`, so a run with the dump on is
        bit-identical to one with it off and `_decide` keeps no knowledge of it — no parameter, no
        branch, no extra continuation in the hot loop. The two rankings could drift apart, which is
        what the gate on the argmax column measures. The pass is affordable because the dump is a
        `Batch_Size = 1` diagnostic."""
        B, Bi, md = market_t1.shape
        K = acts.shape[0]
    ...
    def _decision_curve_row(self, nets, t, market_t1, dF, dL, W, q_prev, q_chosen, live, kappa):
        ...
        *The decision.* `q_prev_net` / `q_chosen_net` are the realized signed covers `Σq·live`,
        ...
        when `Decision_Deadband_Sigma` is armed, the executed book IS the incumbent, and the argmax
        was elsewhere — the band, not agreement, stopped the trade.

        *The utility in force*, the RANKING's rather than the ledger's. `W` is the stepper's net
        wealth entering the decision. `c` is the knee every rung is divided by: the scale off
        `runtime['objective']` at `t+1`, because the rungs are continuations there. It is the
        FRAME's, since `_rollout_on_stepper` builds its stepper with `mirror_scale=False` so a
        loaded checkpoint's scale and schedule survive the roll. `x0` places the day on that
        utility at the point the ranking operates — the wealth LEVEL under `Fixed`, the day's
        INCREMENT under `Running_Wealth`, which is zero before the day happens, so the curvature
        is read off the LOSS side of the reference. `in_knee`, `u_prime` and `local_ARA` follow it,
        and `u_is_increment` says which regime produced them.

        *What the ranking saw.* `curve_net` / `curve_pre` / `curve_charged` are the FULL per-rung
        curve, packed space-separated into three cells so a narrowed ladder does not ragged the
        header. `range_CE` converts the charged curve's spread into dollars at the local `u'`, and
        ...
        chance the day ENDS in the same knee band, per inner draw, at three reference books (flat,
        the standing book, the deepest rung), measured on the same quantity `x0` is.

        *What it was ranking over.* The one-step moments of the fork feeding it, plus a SECOND
        independent fork at the same `t`: `fork_disagree` says whether a rung won on signal or on
        inner-MC noise.

        Conventions. Moments are population (÷N) over the flattened outer × inner draws. `dF_net`
        is the tradable move a UNIT of net cover earns, reduced as `_wealth_step` reduces a book —
        `Σ_i w_i·cs_i·dF_i`, with `w` the `Allocation_Weights` split in force or the uniform split
        over the live, active legs."""
        obj = self.runtime["objective"]
        # the knee the RUNGS divide by: `_score_actions` reads every continuation at t+1, so that
        # is the step whose entry they consume. Under a flat scale the distinction is inert.
        c = _utility_scale(obj, t + 1)
        R = float(obj.get("reference_wealth", 0.0))
        # the knee is huber's; shapes without one take a unit of c as the near-reference band
        delta = (float(obj.get("huber_delta", 1.0))
                 if obj.get("object") == "asymmetricutility_huber" else 1.0)
        w0 = float(W[0])
        # WHERE the ranking operates: the wealth LEVEL under Fixed, the day's INCREMENT under
        # Running_Wealth. Reporting the level under an increment objective logs a curved ranking
        # as a flat one.
        ...
        # a unit of net cover under the composition the schedule mandates
        ...
        # a SECOND fork at the same t: the stream advances, so this is an independent draw of the
        # same conditional law, decided under identical rules
    ...
    def _rollout_on_stepper(self, nets, inner_cache, sweep_ts):
        """Deployment-faithful backtest: roll the frozen policy day-by-day along the bundle's
        observed path through `BundleStepper`, which owns the real futures accounting. Each day
        the book is chosen by `_decide` from the CAUSAL one-step fork forecast (`inner_cache[t]`)
        and the STEPPER'S OWN net wealth, never a verdict wealth recursion, so the decision cannot
        be contaminated by mis-accrued P&L.

        Returns `{greedy, textbook, nohedge}` terminal-P&L stats in the verdict shape, with
        `u_mean_is_step_sum` stating whether `u_mean` is a terminal utility or the running-wealth
        sum. `Solver.DiffV2_Decision_Curve_Dump` hangs a per-decision CSV off the greedy roll; no
        branch below it depends on whether it is on."""
        ...
        def roll(policy):
            ...
            rewards, accumulated here too, over the DECISION days the policy acted on plus the
            tail to the terminal."""
            stepper = BundleStepper(self.bundle, self.runtime, mirror_scale=False)
            # seed the cost-aware decision q_prev from the OPENING book, where the stepper's own
            # positions open too. Position-free it only shifts first-step cost and P&L; under
            # `DiffV2_Position_State` it is the standing book the charge and p' ride.
            q_prev = self.aspace.initial_q(self.B_outer, self.device)
            last = None
            u_sum = w_prev = None
            ...
            while not stepper.done:
                ...
                if stepper.is_decision_step and t in sweep_set:
                    ...
                    if policy == "nohedge":
                        ...
                        # replication clamps per-instrument only, so scale down proportionally to
                        # honour the TOTAL position cap as well
                        tot = float(qt.abs().sum())
                        if self.total_abs_limit > 0.0 and tot > self.total_abs_limit:
                            qt = qt * (self.total_abs_limit / tot)
                        # obey the corridor mandate too, so this is the same in-corridor min-var
                        # benchmark the verdict rolls
                        q = self.aspace.project_to_corridor(qt[None].expand(B, self.n_hedge), t)
                    else:
                        dF, dL, m1, _, live = inner_cache[t]
                        # MODEL-DRIFT CUSUM: the model's one-day forecast minus what the market
                        # then did, summed. Mean-zero under correct assumptions, and standardized
                        ...
                        # informative legs only: live AND still dispersing. On a leg's final day
                        # the fork's dispersion collapses while the settling mark moves, and the
                        # frozen-mark dust over the std clamp fabricates huge sigma readings.
                        ...
                        if tripped and beta > 0.0:
                            # bias the forecast toward the REALIZED drift: the observed average
                            # residual per leg, scaled by beta, shifts the fork moves the ranking
                            # prices
                            adj = torch.tensor(
    ...
    def _check_load_provenance(self, ck, src, md):
        ...
        `DiffV2_Wealth_Free_Value`, each owning an input COLUMN, so a mismatch would otherwise
        surface as a `load_state_dict` shape error naming a Linear weight; the two OBJECTIVE dials
        (`Reference_Mode`, and whether a `conditional_sim` schedule is in force), neither of which
        moves a tensor shape, so a mismatch has no symptom but a quietly wrong policy; a corridor
        mismatch; and, in `_check_action_universe` / `_check_calendar_spread`, a narrowed
        `Allocation_Weights` universe or a `Calendar_Spread_Bps` that moved under position state.

        Warned: a stale `solver_version`, a missing corridor/universe/rate stamp under a live one,
        and a pre-stream frame. Corridor-FREE training spans the widest wealth support, so rolling
        it inside any corridor only restricts to a learned subset; a policy trained INSIDE one is
        off-support under a different or absent corridor."""
        for key, want in (("t_min", self.t_min), ("T_dec", self.T_dec),
            ...
            if ck[key] != want:
                if key in ("t_min", "T_dec") and self.cfg.get("diffv2_load_horizon_pad"):
                    # horizon pad: per-step structures clamp to the saved range at load, and the
                    # tail repeats the last fitted step
        ...
        # roll is a single path and measures none, and the checkpoint's governs it
        ...
        # the training ARCHITECTURE is part of what a version stamps, and an ensemble must not
        # average continuations trained under different ones
    ...
    def _check_action_universe(self, ck, src):
        ...
        if self.cfg.get("diffv2_load_horizon_pad"):
            # a ladder is derived from the run's own calendar, so a knot-for-knot match is
            # impossible under a horizon-padded load and the run's own ladder is searched
    ...
    def _restore_frame(self, members):
        ...
        the checkpoint's `c` is part of the value function, and a per-step SCHEDULE says the same
        on every decision day. The evaluation bundle measured its own at build time and mirrored
        it on; this is where that measurement is discarded. Nothing downstream re-measures — `_bind`
        re-asserts this frame on every later batch and the stepper rollout refuses to re-mirror.

        The scalar is AVERAGED over an ensemble, and under a schedule over the members' TERMINAL
        KNEES, since the scalar is by contract the terminal read. The SCHEDULES themselves are not
        averaged: each member's continuation is read in its own frame, so the run-level schedule is
        the primary member's and only the label and terminal reads consult it."""
        loaded = members[0]
        ...
        # the scalar IS the terminal read, so under a schedule it comes from the members' terminal
        # KNEES rather than a scalar that only happens to equal them
    ...
    def warmup(self, bundle=None):
        ...
        each function's frame, since the train-time standardization stats and utility scale are
        part of the value function and recomputing them from a possibly stressed eval world would
        change what the nets compute. Every eval path is unseen by the nets, so the verdict rolls
        over all paths. A LIST of checkpoints is an ensemble argmax: each member evaluated in its
        own frame, continuations averaged. `_check_load_provenance` holds every member against this
        run first."""
        ...
        # one inner-MC fork per swept t, reused for the argmax, the bootstrap and market_t
        ...
        # standardization stats: market and wealth from the TRAIN swept states, no test peeking
        ...
        # returns-state: the wealth column and its frame live in fractions of the t0 book notional,
        # so a stamped frame travels across price epochs like the market columns do
        ...
        # a load member is either a checkpoint PATH or an already-materialised artifact DICT, so
        ...
        loaded = None
        if load_members:
            # frozen-policy eval: restore the fitted nets AND each member's own frame
            members = []
            versions = set()
            for member in load_members:
                # pre-contract checkpoints still load: only the frame and net keys below are read
        ...
        # per-t trust region for A_t evaluation, set at fit time or restored from a checkpoint;
        # None = unclamped
        ...
        if loaded is not None:
            for net, sd in zip(nets, self._pad_steps(loaded["state_dicts"], self.T_dec)):
                ...
                net.eval()
            if len(members) > 1:
                # ensemble: per-member net stacks and frames; _continuation averages members
                self._ensemble = []
                for ck in members:
        ...
        # the frame is now locked, and streaming freezes the trust region from here on: later
        # batches report their breach rate instead of re-fitting it
    ...
    def step(self, bundle):
        ...
        A LOADED checkpoint means evaluation, so there is nothing to continue: sweeping here would
        fine-tune the "frozen" policy on the evaluation world and report a verdict for a policy
        that is not the checkpoint and is never written anywhere."""
    ...
    def _policy_artifact(self, nets, md, hidden, V_0, n_star_0, worst):
        ...
        return {
            ...
            # the FRAME, so a load restores it rather than re-measuring
            ...
            # u(W) anchor, so the nets are a different function of the same input WIDTH
            ...
            # action-universe provenance: the Allocation_Weights this argmax searched (None = the
            # Cartesian grid). `config_hash` cannot stand in - it hashes runtime['solver'] while
            # this and the corridor live under runtime['accounting'].
            ...
            # charge entering the regressed TARGET, so another rate is a different value function
            "calendar_spread_bps": self.aspace.calendar_bps,
            # position-state provenance: p is an input column, so a load under the other setting is
            # refused BY NAME rather than as a net shape error. wealth_free is the same for W.
            ...
            # bank-policy provenance: WHICH solver class shaped the wealth/position states the nets
            # were fitted on, plus DiffSolverV2's forward-pass frame, so a replay reads the solved
            # construction instead of re-measuring it on different paths
            "solver_object": getattr(self, "SOLVER_OBJECT", "DiffSolver"),
            # the growth reward's shift, measured off the training bank's worst book - part of the
            # FRAME, since a roll under a different shift is a different utility
    ...
    def finish(self, bundle):
        ...
        The POLICY ARTIFACT is built ONCE here: the fitted value function, its frame and the
        provenance stamps. It is the single source returned via `SolverResult` AND torch.saved to
        `DiffV2_Save_Value_Fn`, so the file and the in-memory dict are the same object and the
        eval-from-artifact path is identical to loading the file. A LOADED run produces none —
        nothing was fitted, and re-emitting the file's policy would claim this run as its
        provenance.

        The downside verdict — greedy policy against textbook delta hedge and no hedge — is
        OUT-OF-SAMPLE by construction, since `finish` always runs on a world no fit step saw. With
        a checkpoint loaded and `DiffV2_Stepper_Rollout` set, `_rollout_on_stepper` adds the
        deployment-faithful backtest, which is the trustworthy P&L for a walk-forward because the
        simplified `_verdict` wealth recursion mis-accrues expiry."""
        if bundle is not self.bundle:
            self._bind(bundle)
            # the held-out world needs its OWN forks
            self.inner_cache = {t: self._inner_step(t) for t in self.sweep_ts}
        ...
        # POLICY ARTIFACT, built ONCE here; a LOADED run produces none
        artifact = None
        ...
        # downside verdict, OUT-OF-SAMPLE by construction: all rows of the held-out world
        ...
        # PRIMARY metric = the optimization target E[u(W_T)], which already encodes downside
        # aversion through the concave utility; CVaR5 is a secondary tail diagnostic
...
# `DiffSolver` is the production deliverable; `HindsightDpSolver` is the upper-bound benchmark
# track and `run_textbook_benchmark` the lower-bound one.
class StreamingSolve:
    ...
    The inner-MC fork width is `Batch_Size` rather than the whole simulation, and every fit step
    sees paths no earlier step did, so overfitting to one simulated set is not structurally
    possible. A frozen-policy evaluation is the degenerate stream of length one: nothing to fit,
    so warmup's batch IS the held-out world.
...
def assemble_hedge_result(primary_runs, bundle, runtime):
    ...
    the per-track table and the verdict's `wT_mean` stay comparable."""
    solver_cfg = runtime["solver"]
    ...
    # acceptance ordering - hindsight >= DiffSolver >= textbook over the tracks present, with a
...
class DiffSolverV2(DiffSolver):
    ...
    decision step the MEASURED delta `d_t(book) = P(terminal < max(today's book, 0) | book)` is
    estimated cross-sectionally on the outer paths as a bucketed conditional mean, isotonic
    decreasing in the book mark, ONCE: the strike rides the book, so hedging does not change the
    measured object and the contracts held are known exactly after a single pass. The delta maps
    AFFINELY onto the net band, `q_net = hi − d·(hi − lo)`, and the clairvoyant reserve then adds
    the LEAST extra cover a path needs never to fall under the floor. The delta takes no dial —
    the seed is clairvoyant, so `DiffV2_Risk_Aversion` belongs to the backward DP.

    BACKWARD PASS — the standard sweep, fitted ON the constructed book: the bank's wealth and
    position states are the constructed policy's own roll, plus the usual
    `DiffV2_Bank_Noise_Frac` exploration so the argmax has local support. Every net therefore
    trains where that policy lives, the terminal anchor is the constructed policy's terminal
    distribution, and each earlier net still inherits its fitted successor.

    The per-step Phi curves are FRAME: solved at warmup on the batch that locks the z-frame,
    stamped into the checkpoint beside the nets, and a checkpoint written by a different solver
    class is refused by name, because the bank policy shaped the fitted value."""
    ...
    def _constructed_policy(self, floored=True):
        ...
        `d_t(book) = P(terminal book < book at t | book at t)` is measured on the FLAT liability,
        the only book that exists before a policy does, and the policy rolls once against those
        curves: the strike rides the book, so hedging does not change the measured object and no
        fixed point is needed. Returns `(q per step (B, n_hedge), curves per step, terminal
        book)`."""
        ...
        LT = L[T]
        # THE CLAIRVOYANT RESERVE, backward per path: req_t is the minimum book at t from which the
        # known remaining path can be played without dipping under the floor. Days a futures hedge
        # cannot defend propagate their need BACKWARD, so the forward pass pre-positions.
        ...
        curves = []
        for t in range(T):
            # the strike is the FLOOR, $1/oz over the leg volume. The danger measured is terminal
            # capital breach rather than a high-water ratchet, so a rallying book with no breach
            # risk reads d ~ 0 and the map parks the seed at the TOP of the bound.
            cv = self._phi_curve(L[t], LT < 1.0 * self.leg_volume)
            curves.append(cv)
            if float(rep[t].abs().sum()) < 0.5:
                # less than half a contract of remaining replication delta: the liability is fixed
                # to within dust and the below-today comparison is float noise, so the constructed
                # book is FLAT (the wealth step below still accrues the settling liability rows)
                ...
                # WHOLE contracts, rounded AWAY from zero (up, in cover terms); the box is snapped
                # inward so every integer net is realizable
                net = torch.where(net < 0, net.floor(), net.ceil()).clamp(lo_i, hi_i)
                # THE FLOOR BINDS, made enforceable by the seed's one-day hindsight: where the
                # delta base would let tomorrow's book breach it, hold the smallest position that
                # keeps it above. A day too small to defend keeps the delta base.
                m, dL = m_t[t], L[t + 1] - L[t]
                # tomorrow's book must clear tomorrow's RESERVE, not just the floor. `reqs[t]` IS
                # the requirement at t+1, since the backward loop clones before it updates.
                target = reqs[t]
                if t == 0 or not floored:
                    # t0 is the ONE decision deployment shares with the seed - a single real state
                    # and a single position - so it takes the uniform honest delta with NO
                    # clairvoyance; paths needing more catch up at t1
                    ...
                    continue
                # the floor constraint q*m >= need is a HALF-LINE, and the repair is the delta base
                # PROJECTED onto it: the least change, rounded toward the feasible side. An
                # up-day's bound is a LIMIT on the short - rounding away from zero walks through.
                ...
                # EXECUTION-EXACT top-up: the bound above prices the hedge at the rep-weighted
                # basket move while the executed split deviates, so re-price the EXECUTED book and
                ...
                if bool(fix.any()):
                    # +1 because the exact amount can be re-split away again on the integer
                    # lattice; the floor's guarantee under whole-contract trading is therefore to
                    # within one contract's day move
    ...
    def _build_bank(self, gen):
        ...
        # the floored seed keeps every training book above the floor, so a deep-under-water state
        # would never appear and the net would extrapolate there. Half the paths therefore roll the
        # clairvoyant seed and half the UNDEFENDED delta base.
