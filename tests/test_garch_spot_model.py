...
from derivus.calculation import CMC_State, CMC_State_Inner, construct_process
...
PL_EXP = os.path.join(os.path.dirname(__file__), '..', 'data', 'pl_exp.csv')

# The reference fit below is pinned against a real CME platinum series, which is licensed data and
# is therefore not distributed with the source. Supply your own at data/pl_exp.csv (a date index
# plus a CommodityPrice.PLATINUM column) and these two run; without it they skip rather than fail,
# because a missing input is not a broken calibrator.
needs_pl_exp = pytest.mark.skipif(
    not os.path.exists(PL_EXP), reason=f'no market data at {PL_EXP} - see README, Market data')


def _pl_exp():
    return pd.read_csv(PL_EXP, index_col=0)[['CommodityPrice.PLATINUM']]
