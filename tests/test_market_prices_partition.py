...
alone. The split is now `schema.MARKET_QUOTE_VALUES`: per quote row the mid, its two-way sides and
its `Timestamp` are values, and every other key of the row and everything on the block is
structure. One declaration, read by four call sites, and these gates hold them to it.

WHICH TABLE IS THE QUOTE TABLE is DERIVED rather than listed: `schema.quote_containers` calls a
...
ONE WIRE SPELLING PER TOKEN is the last section, where `.DateOffset` lands: `CustomJsonEncoder`
writes a STRING and `Config.parse_json` wanted a kwargs DICT, so a block this engine wrote could
...
def canonical(obj):
    """The encoder every hash in the engine is taken through, so a round trip surviving it is
...
def loaded(market_prices, deals=None):
    ...
    decoder, so the blocks these gates hash are the ones a job holds."""
...
def family_blocks():
    ...
    Each fixture's row count is asserted HERE, so one arriving empty fails at the source instead of
    turning a refusal gate vacuous - a row-count refusal over zero rows asserts nothing and passes.
    """
    if not _BLOCKS:
        ...
        # everything standing here came from a fixture; the three completed from declarations
        # carry an empty table by construction
...
def test_every_family_partitions_by_the_one_rule_and_round_trips_exactly(family):
    """ROUND TRIP per family: `apply_market_values(*partition_market_price(b))` re-encodes
    byte-identically through the encoder `content_hash` takes every hash through.

    The second half is the UNIFORMITY claim, stated in each family's own table. A family whose
    quote row declares the four value keys has a values half with one entry per row; a family whose
    ...
    if container in VALUE_PLANE:
        ...
        # a PARTITION, not two overlapping projections: every key on exactly one side of the line,
        # which is what makes the round trip above an identity rather than a merge
...
def test_a_short_values_half_refuses_rather_than_dropping_the_rows_it_cannot_pair():
    ...
    a block that would come back shorter than the one it was projected from, with quotes gone and
...
def test_re_authoring_a_quote_set_moves_the_plan(case, plan_too, values_too):
    """The same rule from the other end. A moved pillar or expiry, a moved STRIKE, a quote held out
    with `Use`, a row appended: each re-authors the instrument set the solve is posed over.
    `Weight` is the one worth having beside them - a number that is not a value, because the line is
    drawn at the DECLARED value columns rather than at whether a thing looks like a price.

    `option quote` is the case that flipped: a moved premium is a value on `European_Options`
...
def rows(points, **fields):
    """A `Points` patch naming `fields` on the FIRST row and nothing on the rest - the delta a
    streaming tick sends."""
...
def first_row(context):
    """Row zero of the patched block as it stands NOW: `patch_market` replaces the block rather than
    editing it, so a gate reading a row it captured earlier reads the old one."""
...
def test_a_null_clears_a_two_way_side_and_refuses_on_the_mid():
    """`null` is what a source says "this pillar is no longer quoted two-sided" with, which the
    guard rules is a VALUE. So it clears a side or a `Timestamp` and refuses on
    `Quoted_Market_Value`: a mid is moved, never removed.

    Which keys those are is read off `schema.MARKET_QUOTE_REQUIRED` and every declared value key is
    ...
    # a side plus its stamp in ONE patch, the shape a source that stopped printing a spread sends
    context, points = patchable()
...
def test_a_null_in_a_document_is_an_absence_and_the_values_plane_is_an_identity_over_it(field):
    ...
    the shape that broke it both ways: read as a value present, `market_patch` published the null
    and `quote_delta` read it back as a CLEAR - so the round trip DROPPED a `Quoted_Bid` and moved
    `values_hash` for a no-op, and on a null MID raised against a patch it produced itself.
...
def test_the_tick_guard_and_the_partition_draw_the_same_line():
    """GUARD PARITY, both ways on one pair of doctored blocks: a value-only re-post passes the guard
    AND leaves the structural half identical, and a moved pillar refuses the guard AND moves it.
    `update_market_quote` wrote this split as a refusal and the partition is the same split as a
    projection; they read one tuple, and these are two statements that could disagree.

    THE TICKED BLOCK IS BUILT OFF THE DECLARATION, one mover per member, because a gate naming the
    ...
    # through the real encoder: the guard compares WIRE forms, and a Timestamp reaches it as the
    # token a posted document carries rather than as an object
...
def test_a_patched_quote_reprices_nothing_and_says_so_in_the_hashes():
    """NO-BOOTSTRAP HONESTY: a quote patch moves the quotes and nothing else, so the price factors
...
def test_the_live_book_refuses_a_quote_as_a_values_patch(desk):
    """The one place the book is STRICTER than the engine. `patch_market` takes a quote because an
...
def offsets_in(node, path=''):
    """`[(path, value)]` for every `pd.DateOffset` in a decoded structure, in a stable order. Walked
    rather than named, the ZAR strip carrying its tenors inside authored `Deal` blocks several
    levels down. A `.DateOffset` the decoder left as a raw string or dict is collected as itself,
    which is what lets the type assertion below fail.
    """
...
def test_one_dateoffset_wire_spelling_and_both_decoders_read_it(tmp_path):
    """`CustomJsonEncoder` writes `{'.DateOffset': '3M'}` and `Config.parse_json` did
    `DateOffset(**dct['.DateOffset'])`, which wants a kwargs dict - so a `MarketData.json` this
    engine WROTE could not be read back through `parse_json` at all.

    ONE SPELLING OF THE PARSE, not a second copy: `parse_json` routes the string through
    `Config.parse_period`, the expression `read_json` already used, so the two cannot drift again.
    The kwargs dict is still accepted; reading a spelling nobody writes is free.
...
def test_the_kwargs_dict_still_reads_because_old_bytes_are_on_disk(tmp_path):
    """The legacy spelling keeps working and decodes to the SAME object the string does, which makes
    accepting both a compatibility statement rather than a second convention. Nothing in the tree
    writes it, so this arm is about bytes already on disk.

    A SEPARATE FINDING, OPEN: `CustomJsonEncoder` builds the string by walking `DateOffset.kwds`,
    whose key order for a MULTI-UNIT period is a set iteration and varies with the hash seed -
    ...
    # the string form of the same two offsets, through the same decoder, is the same object
