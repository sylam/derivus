...
WHAT EACH GATE HOLDS, and the numbers it measured on this workstation:
  ...
  1  NESTING, closed form      1.5e-13 relative over 5 strikes x 3 expiries, and 1.2e-10 on the one
                               ...
                               inversion's own cancellation floor)
     NESTING, sub-step         the DRAWS are bitwise identical; the paths agree to 1.0e-15 on the
                               log-spot and 2.6e-15 on the variance
     L curve mechanics         piecewise-linear in t, flat outside its knots, omega AFFINE within a
                               pillar at (B-A)(1-rho)/n per step. Also the certificate's omega_min
                               against the FLAT TAIL past the last knot, the global minimum on a
                               rising strip whose last segment is shorter than 1/(1-rho): 1.006e-6
                               off the strip alone against 1.000e-6 true
     the fitted box            feasible ALGEBRA at every corner - both positivity constraints hold
                               ...
                               point prices: (beta 0.5, l 0.9, a 0.5) is NaN, read as +inf
     the zero-length walk      a kit's FIRST call can be omegas(0, 0) and answered a slice of None:
                               a NaN Value on a row that still looks priced
     quadrature bounds         a phi_max is NOT transferable between contracts - past a contract's
                               ...
                               conservative. A shortcut here mispriced a pillar 0.4%
  ...
  3  the L bootstrap           ATM repriced to 2.4e-15 relative, worst wing 6.88% of premium /
                               0.462 vol points, L within 0.88% of the market's own forward
                               variance strip, q0 == L(0) exactly
  ...
  5  BASEVAL + CREDIT MC       a vanilla prices -0.25% from the component closed form; a one-day
                               TARF (ZERO unmonitored sub-steps) prices finite and keeps its row;
                               a six-fixing TARF carries an 18-row exposure profile with dispersion
                               on every row and a CVA of 2,777.04
  6  both leverage signs       the negative-RR fixture lands Gamma_1 NEGATIVE (-845.1) at an
                               interior optimum
  7  Quote_Sensitivity         REFUSED by name, before any work, naming brentq, the IFT, the
                               roadmap row and the quote chains that ARE differentiable

NO HMC ANYWHERE. Gate 5 is base valuation plus credit Monte Carlo, both through real JSON and the
real entry points.
"""
import os
import sys
...
def test_the_component_closed_form_is_the_plain_one_on_its_nested_face():
    ...
    MACHINE PRECISION, NOT A QUADRATURE TOLERANCE: the parameter map is exact, both pricers hand
    their log-CF to the SAME `cf_european_probabilities`, and both resolve the same adaptive
    `phi_max` - so only accumulation order differs. MEASURED 1.53e-13 worst relative over the grid,
    and 1.15e-10 at its deep out-of-the-money corner (a 21-step 120 strike worth 0.007, where
    8.1e-13 ABSOLUTE is the P1-P2 cancellation's own floor). Hence the `abs` beside the `rel`.

    THE C COEFFICIENT IS WHAT MAKES THIS WORK. The log-CF is A + B*h_0 + C*q_0, and A alone does
    NOT reduce to the plain A - the flat-L intercept `L(1-rho)` is not the plain `omega`. The
    ANCHORING q_0 = L(0) closes it: A + C*L follows the plain A recursion exactly, so dropping the C
    term fails this at every strike.
    """
...
def test_the_component_substep_walks_the_plain_path_draw_for_draw():
    ...
    BITWISE IS UNREACHABLE AND SHOULD BE. The plain step computes
    `omega + beta*h + alpha*(z - g*sh)^2`; the component step computes
    `q + beta*(h - q) + alpha*[(z - g*sh)^2 - (1 + g^2*h)]` with q at L - the same real number on a
    DIFFERENT expression tree, and the centered form is the model's own algebra. What IS bitwise is
    the innovation sequence, so the gate asserts `torch.equal` on the draws and 1e-14 on the paths.

    MEASURED over 40 steps x 4096 paths: draws bitwise identical; worst relative divergence 1.03e-15
    on log S and 2.61e-15 on h, both flat in the step count - the nested recursion is contracting at
    beta = psi = 0.94, so the deviation does not compound.
    """
    plain, comp, level, r = _nesting_pair()
...
def test_the_l_curve_is_piecewise_linear_in_t_and_flat_outside_its_knots():
    ...
    `omega_t = L_(t+1) - rho*L_t` DIFFERENCES this curve: piecewise CONSTANT L would make omega a
    SPIKE at each pillar, and a spline would let it oscillate between pillars nobody quoted.

    LINEAR IN t MAKES OMEGA AFFINE WITHIN A PILLAR, not constant. On a segment of n steps from A to
    B, `omega_i = A(1-rho) + (B-A)(1 + i(1-rho))/n`, drifting by exactly `(B-A)(1-rho)/n` per step
    and KINKING only at a pillar - a second difference of zero inside a segment, which is what this
    asserts along with the slope's closed form.

    A DEGENERATE CURVE - one knot - answers that level everywhere: the flat-L face the nesting gate
    rides.
    """
    ...
    # last knot - 1.006e-6 against 1.000e-6 true
...
def test_every_point_of_the_fitted_box_is_a_feasible_model():
    """Feasible ALGEBRA at every point of the box - both positivity constraints hold by
    construction, so no iterate needs a penalty - AND one corner prices finite. It does NOT assert
    that every point prices: away from the nested face the MGF can diverge and the objective reads
    that candidate as +inf. MEASURED, flat L over 21 steps: (beta 0.5, l 0.9, a 0.5) prices NaN,
    and the (beta 1-1e-6, |l| 1, a 1e-3) corner prices 1.5619.

    TWO CONSTRAINTS, ONE PER SHARE, checked at the box's own corners:

      * the LEVERAGE share holds `Alpha*Gamma_1^2 = |l|*Beta`, so `Beta(1-|l|)` is non-negative -
        what keeps the variance recursion positive;
      * the ARCH share holds `Alpha = a*H0*(1-Beta)`, so `omega_p = H0(1-Beta) - Alpha` is strictly
        positive. Without it the MGF the pricer inverts DIVERGES: the adaptive phi_max scan ran to
        its 2^24 cap and every price came back NaN.

    `unreparam` is its inverse, and the round trip is asserted - it is what a warm start rides. The
    SIGN travels with it both ways, which the negative-risk-reversal fixture needs.
    """
    ...
    # risk: beta at its ceiling, |l| = 1, the ARCH share at its floor - Alpha 7.3e-14 and a
    # dimensionless leverage of 31,623, the box maximum
...
def test_a_quadrature_bound_is_not_transferable_between_contracts():
    """A LARGER phi_max IS NOT CONSERVATIVE for this model. Past a parameter- and step-count-
    dependent point the component A/B/C recursion blows up rather than decaying, so integrating
    beyond a contract's own bound integrates garbage.

    WHAT IT COST: carrying the 21-step contract's bound (512) to the 126-step one (which wants 256)
    solved that pillar's L against a price 0.4% wrong. The ATM ladder is BOOTSTRAPPED and reprices
    exactly by construction, so the only symptom was the report's recompute reading a 3.5e-3
    residual on a ladder that should read 1e-12.

    WHAT THIS HOLDS, at one converged parameter set: the 126-step price is CONVERGED at its own
    bound (identical at 128, 256 and 512, and at 64 against 1024 panels), MOVES at 1024, and is
    nonsense at 2048. And the two contracts in one strip want DIFFERENT bounds: 512 and 256.
    """
...
def test_the_closed_form_matches_day_stepped_monte_carlo():
    ...
    daily recursion, which catches a model right only on its own nested face. The reference is
    `hn_reference.hnc_simulate`, stepping h and q through `utils.hn_component_variance_step` and
    knowing nothing about A/B/C: a European call at three strikes and two expiries, 2^20 paths.

    THE FLOOR MUST NOT BIND, and the gate asserts it. The component recursion has no positivity
    guarantee, the simulator floors and the Fourier inversion does not, so the two are the same law
    only where the floor is inactive.

    MEASURED, 2^20 paths, seed 11: all six cells inside 1.6 standard errors, worst |t| = 1.51 at the
    21-step 100 strike, values 0.284 to 6.225 against standard errors 1.1e-3 to 6.1e-3, floor margin
    2.2e+07 at 21 steps and 9.6e+06 at 63 - seven orders clear.
    """
...
#: `test_service.desk_smile`'s four pillars, as a source posts them. FOUR PILLARS because the ladder
#: needs the ATM term structure to carry more than one expiry, and the RISK REVERSAL IS NEGATIVE in
#: pair terms - on the `FxRate.ZAR` axis a smile RISING with strike, i.e. a NEGATIVE leverage.
...
def _built_surface(points):
    """A book carrying a BUILT `FXVol.USD.ZAR` through the real bootstrap seam, so the component
    gates start from a surface the engine built."""
    document = {'Calc': {
...
def fitted():
    """ONE fit, shared by the round-trip and leverage-sign gates - two readings of one calibration,
    ...
    lands in a different basin: 300 evaluations give Gamma_1 -845.1, worst wing 6.88%, L within
    0.88% of the market strip; 400 give Gamma_1 -78.6, 5.66%, 2.11%. Both are legitimate, which is
    why the gates below hold SHAPE and wide BOUNDS and name the run every number came from."""
...
def test_the_l_bootstrap_reprices_its_atm_ladder_and_lands_on_the_forward_variance_strip(fitted):
    ...
    solver precision rather than fit quality, and L lands on the market's forward variance strip.

    THE ATM RESIDUAL IS THE BOOTSTRAP'S OWN CONVERGENCE - each pillar is a bracketed `brentq` on its
    level against its quote's premium. MEASURED 2.442e-15 worst relative over four pillars, and
    exactly that only because the solve and this recompute derive the same quadrature bound: reusing
    one bound across the ladder read 3.5e-3 here while the solve reported success, which is why this
    is asserted at 1e-8 rather than admired.

    L AGAINST THE MARKET FORWARD VARIANCE STRIP. `E_0[h_t] = L_t` exactly under the anchoring, so
    the model's expected variance over [T_(k-1), T_k] is the sum of L there and the market's is the
    difference of the ATM total variances. MEASURED in annualised vol: model 13.925 / 14.555 /
    14.924 / 15.404 percent against market 13.922 / 14.566 / 14.955 / 15.472 - +0.05 / -0.16 /
    -0.41 / -0.88 percent IN VARIANCE, the model paying for a smile it also has to fit.

    THE GATE HOLDS 8%, an order above that, because the derivative-free search lands in different
    basins at different caps (400 evaluations read +2.37 / +1.40 / +0.88 / +0.15). Still tight
    enough that an L bootstrapped against a TOTAL variance rather than a forward one - the natural
    mistake, 10% out at the back here - fails by multiples.

    THE ANCHORING IS ASSERTED: q_0 IS L(0), read off the written curve's first knot, whose tenor is
    exactly zero. A first knot off zero would leave `HestonNandiComponentImpliedSpotModel` seeding q
    ...
    # L against the market's forward variance strip, segment by segment
...
def test_a_hump_shaped_atm_term_structure_refuses_by_name():
    ...
    THE FIXTURE IS A HUMP: 12% at 1M, 18% at 2M, 19% at 3M, then 10.5% at 6M. No admissible L
    reaches the 6M rung - the least level keeping omega_t >= 0 from A over n steps is
    `A(1 - (1-rho)n / (1 + (n-1)(1-rho)))`, a closed form, so the refusal names the number it wanted.

    WHAT THE MESSAGE CARRIES, each clause a separate assertion: which pillar, the least admissible
    level, the identity that makes it least, the premium it can reach against the one asked for, and
    BOTH remedies (Declining_Variance -> Floor, or a lower Rho).

    AND FLOOR IS THE OTHER HALF: the same fixture under `Declining_Variance: Floor` completes,
    writes a curve with no negative omega, and logs which pillar was floored and by how much it
    therefore misprices.
    """
...
def test_a_rho_at_or_above_one_refuses_by_name():
    ...
    the read is where it is refused.

    q_t is an AR(1) at Rho, so at Rho >= 1 the long-run component is non-stationary and
    `E_0[q_t] = L_t` stops holding. Worse, the declining-variance floor
    `A(1 - (1-Rho)n/(1 + (n-1)(1-Rho)))` goes NEGATIVE at Rho > 1 and `low = max(floor, 1e-12)`
    admits every level the bracket can reach: the negative-omega guard is DISABLED rather than
    tripped, and the family writes a curve whose own simulator drives the variance negative.

    BEFORE ANY WORK, like the Quote_Sensitivity refusal.
    """
    with pytest.raises(ValueError) as refusal:
...
def test_the_negative_risk_reversal_lands_a_negative_leverage_at_an_interior_optimum(fitted):
    ...
    USDZAR's risk reversal is NEGATIVE in pair terms, which on the `FxRate.ZAR` axis is a smile
    RISING with strike. A one-signed leverage could only answer that by switching the channel off
    and calling a flat smile converged; the component family inherits the signed reparametrisation,
    and this proves the inheritance is live rather than copied.

    INTERIOR, TOO: a fit pinned on a bound is the signature of a model that cannot represent its own
    data. MEASURED at the 300-evaluation cap: Alpha 2.720e-07, Beta 0.96409, Gamma_1 -845.12, Phi
    5.438e-08, H0 7.568e-05, whose smallest distance to a bound is 3.6% of that bound's width. At
    400 evaluations the fixture lands in a different basin (Gamma_1 -78.56, margin 17.3%), and the
    SIGN is what both share.
    """
    written, _ = fitted
...
#: A fitted-looking component parameter set, authored into the gate's JSON. These gates are about
#: the PRICERS reaching this model, so the parameters are stated rather than fitted.
COMPONENT_FACTOR = {
...
def _tarf_config(deal, counterparty=False, simulate=False):
    ...
    if simulate:
        # the second seam: the implied factor the pricer consumes also drives the outer scenarios
...
def test_a_vanilla_prices_within_monte_carlo_error_of_the_component_closed_form():
    ...
    A VANILLA IN TARF CLOTHING: one fixing, an unreachable target, no leveraged leg, so the payoff
    is exactly `N (S_T - K)^+` and the OSS machinery reduces to a daily walk to expiry.
    `FXOptionDeal` prices Black off the surface and never reaches a spot model, so a vanilla under
    this model has to be authored as the deal type that carries the switch.
    ...
    # the OSS inner MC's standard error here is ~0.4% at 2^15 paths; a pricer fallen back to GBM at
    # the surface's 14% flat vol reads ~4% out, which is what 2% has to separate
...
def test_the_kit_answers_a_zero_length_walk_before_it_has_a_strip():
    ...
    A fixing interval of one trading day or less walks `n_sub - 1 = 0` unmonitored sub-steps, so all
    four OSS pricers can open a row with `omegas(0, 0)` - and the strip is built LAZILY on a length
    test (`self._built < day + n_steps`) that `0 < 0` answers False. What came back was a slice of
    None: a TypeError `Deal.calculate` swallows, so the symptom is a NaN Value on a row that still
    looks priced.

    The gate holds the kit's contract: an empty strip out, the strip actually built behind it, and
...
def test_a_tarf_whose_first_fixing_is_one_trading_day_out_prices_finite():
    ...
    A one-fixing TARF one calendar day out rounds to `n_sub = 1`, so the pricer asks its kit for
    ZERO unmonitored sub-steps. MEASURED UNDER THE DEFECT: no raise reaches the caller and the row
    is still THERE carrying a Value of NaN, so the gate asserts the row AND the number.

    NO torch.compile PRECONDITION, unlike the 63-day gate beside it: an interval walking no
    unmonitored sub-steps never reaches the fused log sub-step.
    """
...
def test_a_tarf_carries_an_exposure_profile_and_a_finite_cva_under_the_component_model():
    """CREDIT MONTE CARLO, the half base valuation structurally cannot reach: one row per report
    date, one column per path, the component PROCESS driving the underlying and the component PRICER
    valuing the deal on every row.

    THE FAILURE MODE IS THE DEAL BEING SKIPPED - `Deal.calculate` swallows a pricer exception into a
    skipped deal, surfacing only as a missing row much later. A profile collapsed to one row, one
    with no dispersion, or a non-finite CVA all mean the model did not reach the deal. A six-fixing
    TARF walks the OSS truncation and the target latch on every row.

    THE DEAL IS BOUGHT, and that is not cosmetic: CVA is an expectation of POSITIVE exposure, so a
    sold TARF reports a finite CVA of exactly zero and the gate would pass on a model that never ran.

    MEASURED: 18 rows x 64 paths, smallest per-row dispersion 23,885 on a row-zero mean of 344,913,
    CVA 2,777.04.
    """
...
#: Three laws: the USDZAR-shaped fit an FX book carries, an equity-shaped one with the opposite skew
#: sign (so nothing passes on the sign of `gamma_star` alone), and a zero-leverage one - the
#: degenerate face where the map reduces to the unit shift.
...
def test_the_reciprocal_carry_is_the_fx_option_symmetry_in_closed_form():
    """`utils.hn_reciprocal_gamma` in CLOSED FORM, at machine precision, with no Monte Carlo.

    A deal whose underlying IS the base pays on the reciprocal of the only leg the calibration can
    fit AND settles in the other currency - a change of NUMERAIRE, performed by the FxRate itself,
    which shifts the innovation by exactly one standard deviation and turns
    `(omega, alpha, beta, gamma*)` for `s` into `(omega, alpha, beta, 1 - gamma*)` for `1/s`.

    THE IDENTITY IS THE FX OPTION SYMMETRY. At zero rates a put on `s` struck `K` is a call on `1/s`
    struck `1/K` with the notional converted at the strike:

        (1 / K) * E_USD[(K - s)^+]  ==  s0 * E_EUR[(1/s - 1/K)^+]

    Both sides are `utils.hn_call`'s Fourier inversion, so the tolerance is arithmetic.

    MEASURED, worst over three laws x three strikes: 1.4e-12 carried, against `-gamma*` (the sign
    flip without the unit shift) missing by up to 3.2e-2 and the uncarried `gamma*` by up to 5.2 -
    both halves pinned. The Monte Carlo consistency gates cannot do that: on
