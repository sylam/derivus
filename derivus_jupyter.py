    ...
    def __init__(self, path_transform={}, file_transform={}, stressed=False, default_rundate=None):
        ...
        def select_json(chooser):
            ...
            try:
                ...
                logging.error('{} is not a valid derivus JSON file'.format(file_data.name))
            else:
...
if __name__ == '__main__':
    # DERIVUS_MARKETDATA holds dated subdirectories, each carrying a netting_set.json.
    root = os.environ.get('DERIVUS_MARKETDATA', os.path.join(os.path.expanduser('~'), 'marketdata'))
    rundate = os.environ.get('DERIVUS_RUNDATE', '2025-04-07')
    path = os.path.join(root, rundate)
    ...
    cx.load_json(os.path.join(path, 'netting_set.json'))
    rp = SetupPage(cx)
