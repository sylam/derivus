...
Handles the bidirectional translation between:
  - Derivus's nested deal tree (Deal objects, Curve, Percent, etc.)
...
def _make_as_internal():
    """Return an object_hook that decodes derivus JSON special types."""
    cfg = _get_cfg()
...
def decode_fields_json(fields_json: str) -> dict[str, Any]:
    """Decode a Fields_JSON string to a derivus-compatible field dict."""
...
def _to_cell(v: Any) -> Any:
    """Convert a derivus field value to an Excel-friendly scalar or JSON string."""
